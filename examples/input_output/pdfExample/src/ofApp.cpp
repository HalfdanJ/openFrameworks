...
void ofApp::draw() {
	...
	if(oneShot || pdfRendering) {
		...
		font.drawStringAsShapes(ofToString(ofGetFrameNum()),
		                        32 + font.getStringBoundingBox("Current Frame: ", 0, 0).width + 9, 500);
	} else {
		font.drawString("Current Frame: ", 32, 500);
		ofSetColor(245, 58, 135);
		font.drawString(ofToString(ofGetFrameNum()), 32 + font.getStringBoundingBox("Current Frame: ", 0, 0).width + 9,
		                500);
	}
