...
void ofApp::update() {
	...
	for(int i = 0; i < kNumTestNodes; i++) {
		testNodes[i].setPosition(ofVec3f(sin(ofGetElapsedTimef() * freqMult) * amp,
...
void ofApp::draw() {
	...
	string s = string("") + "\n" +
	           "Purple boxes (4 of them) are generic nodes with simple circular motion, linked in a hierarchy (with "
	           "ofNode::setParent).\n" +
	           "Yellow boxes (2 of them) are cameras. You are looking through one of them so can only see one box on "
	           "screen.\n" +
	           "\n" + "KEYS:\n" + "\n" + "z reset transforms\n" + "\n" +
	...
	for(int i = 0; i < kNumCameras; i++) {
		...
		if(lookatIndex[i] >= 0)
			cam[i].lookAt(testNodes[lookatIndex[i]], {0.f, 1.f, 0.f});
...
void ofApp::keyPressed(int key) {
	...
	cout << "X-AXIS: " + ofToString(n->getXAxis().x) + " " + ofToString(n->getXAxis().y) + " " +
	            ofToString(n->getXAxis().z) + "\n";
	cout << "Y-AXIS: " + ofToString(n->getYAxis().x) + " " + ofToString(n->getYAxis().y) + " " +
	            ofToString(n->getYAxis().z) + "\n";
	cout << "Z-AXIS: " + ofToString(n->getZAxis().x) + " " + ofToString(n->getZAxis().y) + " " +
	            ofToString(n->getZAxis().z) + "\n";
}
