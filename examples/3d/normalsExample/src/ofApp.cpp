...
void ofApp::setup() {
	...
	for(int i = 0; i < max; i++) {
		...
		float theta =
		    ofMap(i, 0, max - 1, 0,
		          2 * PI - step); // map i as circle divisions to actual radian values around the circle (note we don't
		                          // go quite all the way around by one step, because it will be the same as where we
		                          // started, so we'll just index that starting vertex when we make faces)
		...
		/* notice here we go in the same direction: previous->current,current->next;
		   ...
		   this might not be the best idea, but its certainly better than going previous->current,next->current, which
		   would end up being quite awful. This is the concept of an "orientable mesh" or "face winding order", to be
		   googled for more information.
		 */

		// since we want smooth normals, we'll sum the two adjacent face normals, then normalize (since usually, only
		// the direction and not the magnitude of the normal is what matters)
...
void ofApp::draw() {
	...
	if(!ofGetKeyPressed()) {
		...
		for(unsigned int i = 0; i < n.size(); i++) {
			ofDrawLine(v[i].x, v[i].y, v[i].z, v[i].x + n[i].x * normalLength, v[i].y + n[i].y * normalLength,
			           v[i].z + n[i].z * normalLength);

			ofDrawLine(.98 * v[i].x, .98 * v[i].y, .98 * v[i].z, .98 * v[i].x + n[i].x * normalLength * .2,
			           .98 * v[i].y + n[i].y * normalLength * .2, .98 * v[i].z + n[i].z * normalLength * .2);
			ofDrawLine(.98 * v[i].x + n[i].x * normalLength * .2, .98 * v[i].y + n[i].y * normalLength * .2,
			           .98 * v[i].z + n[i].z * normalLength * .2, v[i].x + n[i].x * normalLength * .2,
			           v[i].y + n[i].y * normalLength * .2, v[i].z + n[i].z * normalLength * .2);
		}
	}
