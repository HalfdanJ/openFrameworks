...
// This are actually not compiled, instead whenever you use a defined directive it gets replaced by it's associated
// value in the source code BEFORE it compiles. So this don't use memory space and if you are using this directives a lot
// it's easier and faster to change it's value.
...
public:
	...
	oscillator() {}
	~oscillator() {}
	void setup(int x, int y) { // this function initializes all the properties of the oscillator. It must be called just
		                       // after creating a new oscillator.

		pos.set(x, y); // sets the position of the oscillator using the values passed to the function. In this case is
		               // the position where the mouse was clicked.
		freq = ofRandom(0.0001f, 0.1f); // the frequency get's initialized with a random value.

		counter = 0;
		if(y < TOP_MARGIN) { // this if is to determine if the oscillator is either an horizontal or vertical
			                 // oscillator.
			phase = ofMap(x - LEFT_MARGIN, LEFT_MARGIN, ofGetWidth(), -PI, PI,
			              false); // the phase is defined by to position of the locator.Here it's value gets remaped to
			                      // the correct range.

			amplitude =
			    ofMap(y, 0, TOP_MARGIN, 0.1f, 300,
			          false); // Amplitude is defined by the position, and it's value gets remaped to the correct range.
	...
	void update() {
		counter +=
		    freq; // the counter is incremented by the value of freq each time the oscillators update methods is called.
...
public:
	...
	ofVec3f waveHistory[TAIL_LENGTH]; // This array object is to save the previous positions of the composite wave we're
	                                  // creating, so we can draw it's path later.
	...
	bool bSelectedOscHor;  // when you click over an oscillator this boolean indicates whether it was one of the
	                      // horizontal oscillators or not.
	bool bSelectedOscVert; // when you click over an oscillator this boolean indicates whether it was one of the
	                       // vertical oscillators or not.
};
