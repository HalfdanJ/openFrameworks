...
void ofApp::setup() {
	...
	center =
	    ofPoint((ofGetWidth() - LEFT_MARGIN) * 0.5f + LEFT_MARGIN, (ofGetHeight() - TOP_MARGIN) * 0.5f + TOP_MARGIN);
...
void ofApp::draw() {
	...
	ofMesh wave; // declaring a new ofMesh object with which we're drawing the motion path created by summing the
	...
	wave.draw();
	ofSetColor(0, 10, 255), ofDrawCircle(horWave, vertWave, 10);
	ofPopMatrix();
}
...
void ofApp::mouseDragged(int x, int y, int button) {
	if(selectedOscilator >
	   -1) { // when an oscillator is clicked and dragged it's speed (freq) and amplitude are changed.
		// the following lines do so.
		if(bSelectedOscHor == true) {
			horizontalOscilators[selectedOscilator].freq +=
			    0.1f * (ofGetPreviousMouseX() - ofGetMouseX()) / float(ofGetWidth());
			horizontalOscilators[selectedOscilator].amplitude += ofGetMouseY() - ofGetPreviousMouseY();
		} else if(bSelectedOscVert == true) {
			verticalOscilators[selectedOscilator].freq +=
			    0.1f * (ofGetPreviousMouseY() - ofGetMouseY()) / float(ofGetHeight());
...
void ofApp::mousePressed(int x, int y, int button) {
	...
	if(y < TOP_MARGIN && x > LEFT_MARGIN) {
		for(unsigned int i = 0; i < horizontalOscilators.size();
		    i++) { // this goes through the horizontal oscillators checking if anyone has been clicked over.
			if(horizontalOscilators[i].checkOver(x, y)) {
				setPressedOscilator(i, true);
				break;
			}
		}
		if(!bSelectedOscHor) { // in case that no oscillator was clicked then create a new one at the position of the
			                   // mouse.
