...
void demoParticle::update() {
	...
	if(mode == PARTICLE_MODE_ATTRACT) {
		...
		if(dist < 150) {
			...
			// if the particles are not close to us, lets add a little bit of random movement using noise. this is where
			// uniqueVal comes in handy.
		...
		// we add pos.y as an arg so to prevent obvious vertical banding around x values - try removing the pos.y *
		// 0.006 to see the banding
		...
		// we do this so as to skip the bounds check for the bottom and make the particles go back to the top of the
		// screen
		...
		if(attractPoints) {
			...
			if(closest != -1) {
				...
				if(dist < 300 && dist > 40 && !ofGetKeyPressed('f')) {
					...
					// if the particles are not close to us, lets add a little bit of random movement using noise. this
					// is where uniqueVal comes in handy.
					frc.x = ofSignedNoise(uniqueVal, pos.y * 0.01, ofGetElapsedTimef() * 0.2);
					frc.y = ofSignedNoise(uniqueVal, pos.x * 0.01, ofGetElapsedTimef() * 0.2);
					vel += frc * 0.4;
				}
