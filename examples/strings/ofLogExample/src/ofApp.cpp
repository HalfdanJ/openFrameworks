...
void ofApp::update() {
	...
	if(elapsedTime >= triggerTwo) {
		ofLog(OF_LOG_NOTICE, "5s timer: Run time is %i seconds",
		      elapsedTime / 1000); // Another way of composing log messages
	...
	if(verboseLevel != currentVerboseLevel) { // if the verbosity level has just changed
		...
		ofLog(OF_LOG_FATAL_ERROR,
		      "New log level is " + ofGetLogLevelName(ofGetLogLevel())); // The third way of composing log messages
	}
