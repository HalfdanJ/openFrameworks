...
void ofApp::setup() {
	...
	ofAddListener(
	    redCircle.clickedInside, // the ofEvent that we want to listen to. In this case exclusively to the circleEvent
	                             // of redCircle (red circle) object.
	    this, // pointer to the class that is going to be listening. it can be a pointer to any object. There's no need
	          // to declare the listeners within the class that's going to listen.
	    &ofApp::onMouseInCircle); // pointer to the method that's going to be called when a new event is broadcasted
	                              // (callback method). The parameters of the event are passed to this method.
	ofAddListener(
	    Circle::clickedInsideGlobal, this,
	    &ofApp::onMouseInAnyCircle); // listening to this event will enable us to get events from any instance of the
	                                 // circle class as this event is static (shared by all instances of the same class).
