...
int main() {
	...
	if(ofStringTimesInString(extensionsAvailable, "GL_ARB_draw_instanced") == 0) {
		ofLogFatalError("App") << " GL_ARB_draw_instanced is needed for this example but it is not supported by your "
		                          "graphics card. Exiting App.";
		return -1;
		...
		if(ofStringTimesInString(extensionsAvailable, "GL_EXT_gpu_shader4") == 0 &&
		   ofStringTimesInString(extensionsAvailable, "NV_vertex_program4") == 0) {
			ofLogFatalError("App") << " GL_EXT_gpu_shader4 or NV_vertex_program4 is needed for this example but it is "
			                          "not supported by your graphics card. Exiting App.";
			return -1;
		}
	}
#endif
