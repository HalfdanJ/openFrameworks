...
void ofThread::waitForThread(bool callStopThread, long milliseconds) {
	if(!threadDone) {
		...
		if(INFINITE_JOIN_TIMEOUT == milliseconds) {
			...
			std::unique_lock<std::mutex> lck(mutex);
			if(!threadDone &&
			   condition.wait_for(lck, std::chrono::milliseconds(milliseconds)) == std::cv_status::timeout) {
...
void ofThread::threadedFunction() {
	ofLogWarning("ofThread") << "- name: " << getThreadName()
	                         << " - Override ofThread::threadedFunction() in your ofThread subclass.";
}
