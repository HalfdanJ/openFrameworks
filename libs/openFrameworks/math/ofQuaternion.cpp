...
void ofQuaternion::makeRotate(float angle1, const ofVec3f &axis1, float angle2, const ofVec3f &axis2, float angle3,
                              const ofVec3f &axis3) {
	ofQuaternion q1;
...
void ofQuaternion::makeRotate_original(const ofVec3f &from, const ofVec3f &to) {
	...
	if(fabs(cosangle - 1) < epsilon) {
		// osg::notify(osg::INFO)<<"*** Quat::makeRotate(from,to) with near co-linear vectors, epsilon=
		// "<<fabs(cosangle-1)<<std::endl;
