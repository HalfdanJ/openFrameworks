...
public:
	...
	/// \param tolerance an angle tolerance/threshold (specified in degrees) for deciding if the vectors are
	/// sufficiently aligned. \returns true if both vectors are aligned (pointing in the same direction).
	...
	/// \param vec The vector to compare alignment with
	/// \param tolerance an angle tolerance/threshold (specified in radians) for deciding if the vectors are
	/// sufficiently aligned. \sa isAligned()
...
inline ofVec2f ofVec2f::getRotated(float angle) const {
	...
	return ofVec2f(x * cos(a) - y * sin(a), x * sin(a) + y * cos(a));
}
...
inline ofVec2f ofVec2f::mapped(const ofVec2f &origin, const ofVec2f &vx, const ofVec2f &vy) const {
	return getMapped(origin, vx, vy);
}

inline ofVec2f ofVec2f::getMapped(const ofVec2f &origin, const ofVec2f &vx, const ofVec2f &vy) const {
	return ofVec2f(origin.x + x * vx.x + y * vy.x, origin.y + x * vx.y + y * vy.y);
}

inline ofVec2f &ofVec2f::map(const ofVec2f &origin, const ofVec2f &vx, const ofVec2f &vy) {
