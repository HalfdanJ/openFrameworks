...
float ofMatrix3x3::determinant() const {
	float det = a * e * i + b * f * g + d * h * c - g * e * c - d * b * i - h * f * a;
	return det;
}
...
void ofMatrix3x3::invert() {
	...
	// included in these calculations: minor, cofactor (changed signs), transpose (by the order of "="), division
	// through determinant
	B.a = (e * i - h * f) / det;
	B.b = (-b * i + h * c) / det;
	B.c = (b * f - e * c) / det;
...
ostream &operator<<(ostream &os, const ofMatrix3x3 &M) {
	int w = 8;
	os << setw(w) << M.a << ", " << setw(w) << M.b << ", " << setw(w) << M.c << std::endl;

	os << setw(w) << M.d << ", " << setw(w) << M.e << ", " << setw(w) << M.f << std::endl;
