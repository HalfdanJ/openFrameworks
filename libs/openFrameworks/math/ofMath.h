...
bool ofLineSegmentIntersection(const vectype &line1Start, const vectype &line1End, const vectype &line2Start,
                               const vectype &line2End, vectype &intersection) {
...
vectype ofCurvePoint(const vectype &a, const vectype &b, const vectype &c, const vectype &d, float t) {
	...
	float t2 = t * t;
	float t3 = t2 * t;
	pt.x = 0.5f * ((2.0f * b.x) + (-a.x + c.x) * t + (2.0f * a.x - 5.0f * b.x + 4 * c.x - d.x) * t2 +
	               (-a.x + 3.0f * b.x - 3.0f * c.x + d.x) * t3);
	pt.y = 0.5f * ((2.0f * b.y) + (-a.y + c.y) * t + (2.0f * a.y - 5.0f * b.y + 4 * c.y - d.y) * t2 +
	               (-a.y + 3.0f * b.y - 3.0f * c.y + d.y) * t3);
	return pt;
}

/// Given the four points that determine a bezier curve and an offset along the curve, return an tangent vector to a
/// point on the curve. Currently this is not a normalized point, and will need to be normalized. \param a The beginning
/// point of the curve. \param b The first control point. \param c The second control point. \param d The end point of
/// the curve. \param t an offset along the curve, normalized between 0 and 1. \returns A glm::vec3 on the curve.
template<class vectype>
...
template<typename Type> Type ofInterpolateCosine(const Type &y1, const Type &y2, float pct);
...
template<typename Type>
Type ofInterpolateHermite(const Type &y0, const Type &y1, const Type &y2, const Type &y3, float pct, float tension,
                          float bias);
