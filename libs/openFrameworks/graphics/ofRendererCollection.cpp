...
bool ofRendererCollection::rendersPathPrimitives() {
	return true;
}
...
void ofRendererCollection::draw(const ofMesh &vertexData, ofPolyRenderMode mode, bool useColors, bool useTextures,
...
void ofRendererCollection::draw(const ofImage &img, float x, float y, float z, float w, float h, float sx, float sy,
...
void ofRendererCollection::draw(const ofFloatImage &img, float x, float y, float z, float w, float h, float sx,
...
void ofRendererCollection::draw(const ofShortImage &img, float x, float y, float z, float w, float h, float sx,
                                float sy, float sw, float sh) const {
	for(auto renderer : renderers) {
		renderer->draw(img, x, y, z, w, h, sx, sy, sw, sh);
	}
...
glm::mat4 ofRendererCollection::getCurrentMatrix(ofMatrixMode matrixMode_) const {
	if(!renderers.empty()) {
		...
		ofLogWarning()
		    << "No renderer in renderer collection, but current matrix requested. Returning identity matrix.";
...
glm::mat4 ofRendererCollection::getCurrentViewMatrix() const {
	if(!renderers.empty()) {
		...
		ofLogWarning()
		    << "No renderer in renderer collection, but current view matrix requested. Returning identity matrix.";
		return glm::mat4(1.0);
	}
}
...
void ofRendererCollection::drawTriangle(float x1, float y1, float z1, float x2, float y2, float z2, float x3, float y3,
                                        float z3) const {
	for(auto renderer : renderers) {
