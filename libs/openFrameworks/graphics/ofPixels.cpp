...
template<typename PixelType> size_t ofPixels_<PixelType>::pixelBitsFromPixelFormat(ofPixelFormat format) {
	switch(format) {
	case OF_PIXELS_RGB:
	case OF_PIXELS_BGR:
...
template<> std::string ofToString(const ofPixelFormat &p) {
	switch(p) {
	case OF_PIXELS_GRAY:
...
template<typename PixelType> ofPixels_<PixelType>::ofPixels_() {}

template<typename PixelType> ofPixels_<PixelType>::~ofPixels_() {
	clear();
}

template<typename PixelType> ofPixels_<PixelType>::ofPixels_(const ofPixels_<PixelType> &mom) {
	copyFrom(mom);
}
...
template<typename PixelType> void ofPixels_<PixelType>::swap(ofPixels_<PixelType> &pix) {
...
template<typename PixelType> ofPixels_<PixelType> &ofPixels_<PixelType>::operator=(const ofPixels_<PixelType> &mom) {
	if(this == &mom) {
		return *this;
	}
	copyFrom(mom);
...
template<typename PixelType> ofPixels_<PixelType> &ofPixels_<PixelType>::operator=(ofPixels_<PixelType> &&mom) {
	if(this == &mom) {
		return *this;
	}
	clear();
...
template<typename PixelType> void ofPixels_<PixelType>::copyFrom(const ofPixels_<PixelType> &mom) {
	if(mom.isAllocated()) {
...
template<typename PixelType> void ofPixels_<PixelType>::set(PixelType val) {
	iterator _end = end();
...
template<typename PixelType> void ofPixels_<PixelType>::set(size_t channel, PixelType val) {
	switch(pixelFormat) {
	case OF_PIXELS_RGB:
	case OF_PIXELS_BGR:
	case OF_PIXELS_RGBA:
...
template<typename PixelType>
void ofPixels_<PixelType>::setFromAlignedPixels(const PixelType *newPixels, size_t width, size_t height,
...
template<typename PixelType>
void ofPixels_<PixelType>::setFromAlignedPixels(const PixelType *newPixels, size_t width, size_t height,
                                                ofPixelFormat _pixelFormat, size_t stride) {
...
template<typename PixelType>
void ofPixels_<PixelType>::setFromAlignedPixels(const PixelType *newPixels, size_t width, size_t height,
                                                ofPixelFormat _pixelFormat, std::vector<size_t> strides) {
	size_t channels = channelsFromPixelFormat(_pixelFormat);
	if(channels == 0)
		return;
...
template<typename PixelType> PixelType *ofPixels_<PixelType>::getPixels() {
	return pixels;
}

template<typename PixelType> const PixelType *ofPixels_<PixelType>::getPixels() const {
	return pixels;
}

template<typename PixelType> PixelType *ofPixels_<PixelType>::getData() {
	return pixels;
}

template<typename PixelType> const PixelType *ofPixels_<PixelType>::getData() const {
	return pixels;
}

template<typename PixelType> void ofPixels_<PixelType>::allocate(size_t w, size_t h, size_t _channels) {
	allocate(w, h, pixelFormatFromNumChannels(_channels));
}

template<typename PixelType> void ofPixels_<PixelType>::allocate(size_t w, size_t h, ofPixelFormat format) {
...
template<typename PixelType> void ofPixels_<PixelType>::allocate(size_t w, size_t h, ofImageType type) {
	allocate(w, h, ofPixelFormatFromImageType(type));
}

template<typename PixelType> void ofPixels_<PixelType>::swapRgb() {
	switch(pixelFormat) {
	case OF_PIXELS_RGB:
	case OF_PIXELS_BGR:
	case OF_PIXELS_RGBA:
...
template<typename PixelType> void ofPixels_<PixelType>::clear() {
	if(pixels) {
		if(pixelsOwner)
			delete[] pixels;
		pixels = nullptr;
...
template<typename PixelType> size_t ofPixels_<PixelType>::getPixelIndex(size_t x, size_t y) const {
	if(!bAllocated) {
		return 0;
	} else {
		size_t pixelStride;
...
template<typename PixelType> ofColor_<PixelType> ofPixels_<PixelType>::getColor(size_t index) const {
	return (Pixel(pixels + index, getNumChannels(), pixelFormat)).getColor();
}

template<typename PixelType> ofColor_<PixelType> ofPixels_<PixelType>::getColor(size_t x, size_t y) const {
	return getColor(getPixelIndex(x, y));
}

template<typename PixelType> void ofPixels_<PixelType>::setColor(size_t index, const ofColor_<PixelType> &color) {
...
template<typename PixelType> void ofPixels_<PixelType>::setColor(size_t x, size_t y, const ofColor_<PixelType> &color) {
	setColor(getPixelIndex(x, y), color);
}

template<typename PixelType> void ofPixels_<PixelType>::setColor(const ofColor_<PixelType> &color) {
	switch(pixelFormat) {
	case OF_PIXELS_RGB: {
		for(auto pixel : getPixelsIter()) {
			pixel[0] = color.r;
...
template<typename PixelType> PixelType &ofPixels_<PixelType>::operator[](size_t pos) {
	return pixels[pos];
}

template<typename PixelType> const PixelType &ofPixels_<PixelType>::operator[](size_t pos) const {
	return pixels[pos];
}

template<typename PixelType> bool ofPixels_<PixelType>::isAllocated() const {
	return bAllocated;
}

template<typename PixelType> size_t ofPixels_<PixelType>::getWidth() const {
	return width;
}

template<typename PixelType> size_t ofPixels_<PixelType>::getHeight() const {
	return height;
}

template<typename PixelType> size_t ofPixels_<PixelType>::getBytesPerPixel() const {
	return pixelBitsFromPixelFormat(pixelFormat) / 8;
}

template<typename PixelType> size_t ofPixels_<PixelType>::getBitsPerPixel() const {
	return pixelBitsFromPixelFormat(pixelFormat);
}

template<typename PixelType> size_t ofPixels_<PixelType>::getBytesPerChannel() const {
	return sizeof(PixelType);
}

template<typename PixelType> size_t ofPixels_<PixelType>::getBitsPerChannel() const {
	return getBytesPerChannel() * 8;
}

template<typename PixelType> size_t ofPixels_<PixelType>::getBytesStride() const {
	return pixelBitsFromPixelFormat(pixelFormat) * width / 8;
}

template<typename PixelType> size_t ofPixels_<PixelType>::getNumChannels() const {
...
template<typename PixelType> size_t ofPixels_<PixelType>::getNumPlanes() const {
	switch(pixelFormat) {
	case OF_PIXELS_RGB:
	case OF_PIXELS_BGR:
	case OF_PIXELS_RGB565:
...
template<typename PixelType> ofPixels_<PixelType> ofPixels_<PixelType>::getPlane(size_t planeIdx) {
	...
	case OF_PIXELS_YV12:
		...
		case 1:
			...
			break;
		case 2:
			plane.setFromExternalPixels(pixels + (width * height + width / 2 * height / 2), width / 2, height / 2,
			                            OF_PIXELS_U);
			break;
		}
		break;
...
template<typename PixelType> ofImageType ofPixels_<PixelType>::getImageType() const {
	return ofImageTypeFromPixelFormat(pixelFormat);
}

template<typename PixelType> void ofPixels_<PixelType>::setImageType(ofImageType imageType) {
	if(!isAllocated() || imageType == getImageType())
		return;
...
template<typename PixelType> ofPixelFormat ofPixels_<PixelType>::getPixelFormat() const {
	return pixelFormat;
}

template<typename PixelType> void ofPixels_<PixelType>::setNumChannels(size_t numChannels) {
	if(!isAllocated() || numChannels == getNumChannels())
		return;
	setImageType(getImageTypeFromChannels(numChannels));
}

template<typename PixelType> size_t ofPixels_<PixelType>::size() const {
	return pixelsSize;
}

template<typename PixelType> ofPixels_<PixelType> ofPixels_<PixelType>::getChannel(size_t channel) const {
...
// From ofPixelsUtils
//----------------------------------------------------------------------
template<typename PixelType> void ofPixels_<PixelType>::crop(size_t x, size_t y, size_t _width, size_t _height) {
	if(bAllocated) {
...
//----------------------------------------------------------------------
template<typename PixelType>
void ofPixels_<PixelType>::cropTo(ofPixels_<PixelType> &toPix, size_t x, size_t y, size_t _width,
                                  size_t _height) const {
	if(bAllocated) {
...
//----------------------------------------------------------------------
template<typename PixelType> void ofPixels_<PixelType>::rotate90(int nClockwiseRotations) {
	size_t channels = channelsFromPixelFormat(pixelFormat);
...
//----------------------------------------------------------------------
template<typename PixelType> void ofPixels_<PixelType>::mirror(bool vertically, bool horizontal) {
	size_t channels = channelsFromPixelFormat(pixelFormat);
...
//----------------------------------------------------------------------
template<typename PixelType>
float ofPixels_<PixelType>::bicubicInterpolate(const float *patch, float x, float y, float x2, float y2, float x3,
                                               float y3) {
	...
	float a22 = 4 * p00 - 4 * p01 + 2.0f * p02 - 2.0f * p03 - 4 * p10 + 4 * p11 - 2.0f * p12 + 2.0f * p13 + 2.0f * p20 -
	            2.0f * p21 + p22 - p23 - 2.0f * p30 + 2.0f * p31 - p32 + p33;
	float a23 = -2.0f * p00 + 2.0f * p01 - 2.0f * p02 + 2.0f * p03 + 2.0f * p10 - 2.0f * p11 + 2.0f * p12 - 2.0f * p13 -
	            p20 + p21 - p22 + p23 + p30 - p31 + p32 - p33;
	float a30 = -p01 + p11 - p21 + p31;
	float a31 = p00 - p02 - p10 + p12 + p20 - p22 - p30 + p32;
	float a32 = -2.0f * p00 + 2.0f * p01 - p02 + p03 + 2.0f * p10 - 2.0f * p11 + p12 - p13 - 2.0f * p20 + 2.0f * p21 -
	            p22 + p23 + 2.0f * p30 - 2.0f * p31 + p32 - p33;
	float a33 = p00 - p01 + p02 - p03 - p10 + p11 - p12 + p13 + p20 - p21 + p22 - p23 - p30 + p31 - p32 + p33;

	float out = a00 + a01 * y + a02 * y2 + a03 * y3 + a10 * x + a11 * x * y + a12 * x * y2 + a13 * x * y3 + a20 * x2 +
	            a21 * x2 * y + a22 * x2 * y2 + a23 * x2 * y3 + a30 * x3 + a31 * x3 * y + a32 * x3 * y2 + a33 * x3 * y3;
...
bool ofPixels_<PixelType>::resizeTo(ofPixels_<PixelType> &dst, ofInterpolationMethod interpMethod) const {
	...
	if(!(isAllocated()) || !(dst.isAllocated()) || getBytesPerPixel() != dst.getBytesPerPixel())
		return false;
...
bool ofPixels_<PixelType>::pasteInto(ofPixels_<PixelType> &dst, size_t xTo, size_t yTo) const {
	if(!(isAllocated()) || !(dst.isAllocated()) || getBytesPerPixel() != dst.getBytesPerPixel() ||
	   xTo + getWidth() > dst.getWidth() || yTo + getHeight() > dst.getHeight())
		return false;

	size_t bytesToCopyPerRow =
	    (xTo + getWidth() <= dst.getWidth() ? getWidth() : dst.getWidth() - xTo) * getBytesPerPixel();
...
template<typename A, typename B> inline A clampedAdd(const A &a, const B &b) {
...
bool ofPixels_<PixelType>::blendInto(ofPixels_<PixelType> &dst, size_t xTo, size_t yTo) const {
	if(!(isAllocated()) || !(dst.isAllocated()) || getBytesPerPixel() != dst.getBytesPerPixel() ||
	   xTo + getWidth() > dst.getWidth() || yTo + getHeight() > dst.getHeight() || getNumChannels() == 0)
		return false;

	std::function<void(const ConstPixel &, Pixel &)> blendFunc;
	switch(getNumChannels()) {
	case 1:
		blendFunc = [](const ConstPixel &src, Pixel &dst) { dst[0] = clampedAdd(src[0], dst[0]); };
		break;
	case 2:
		blendFunc = [](const ConstPixel &src, Pixel &dst) {
			dst[0] =
			    clampedAdd(src[0], dst[0] / ofColor_<PixelType>::limit() * (ofColor_<PixelType>::limit() - src[1]));
			dst[1] =
			    clampedAdd(src[1], dst[1] / ofColor_<PixelType>::limit() * (ofColor_<PixelType>::limit() - src[1]));
		};
		break;
	case 3:
		blendFunc = [](const ConstPixel &src, Pixel &dst) {
	...
	case 4:
		blendFunc = [](const ConstPixel &src, Pixel &dst) {
			dst[0] =
			    clampedAdd(src[0], dst[0] / ofColor_<PixelType>::limit() * (ofColor_<PixelType>::limit() - src[3]));
			dst[1] =
			    clampedAdd(src[1], dst[1] / ofColor_<PixelType>::limit() * (ofColor_<PixelType>::limit() - src[3]));
			dst[2] =
			    clampedAdd(src[2], dst[2] / ofColor_<PixelType>::limit() * (ofColor_<PixelType>::limit() - src[3]));
			dst[3] =
			    clampedAdd(src[3], dst[3] / ofColor_<PixelType>::limit() * (ofColor_<PixelType>::limit() - src[3]));
		};
		break;
	}
