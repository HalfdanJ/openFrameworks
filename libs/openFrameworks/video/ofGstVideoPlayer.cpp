...
bool ofGstVideoPlayer::createPipeline(string name) {
	...
	case OF_PIXELS_GRAY:
		bpp = 8;
		caps = gst_caps_new_simple("video/x-raw-gray", "bpp", G_TYPE_INT, bpp, "depth", G_TYPE_INT, 8, NULL);
		break;
	case OF_PIXELS_RGB:
		bpp = 24;
		caps = gst_caps_new_simple("video/x-raw-rgb", "bpp", G_TYPE_INT, bpp, "depth", G_TYPE_INT, 24, "endianness",
		                           G_TYPE_INT, 4321, "red_mask", G_TYPE_INT, 0xff0000, "green_mask", G_TYPE_INT,
		                           0x00ff00, "blue_mask", G_TYPE_INT, 0x0000ff, NULL);
		break;
	case OF_PIXELS_RGBA:
		bpp = 32;
		caps = gst_caps_new_simple("video/x-raw-rgb", "bpp", G_TYPE_INT, bpp, "depth", G_TYPE_INT, 32, "endianness",
		                           G_TYPE_INT, 4321, "red_mask", G_TYPE_INT, 0xff000000, "green_mask", G_TYPE_INT,
		                           0x00ff0000, "blue_mask", G_TYPE_INT, 0x0000ff00, "alpha_mask", G_TYPE_INT,
		                           0x000000ff, NULL);
		break;
	case OF_PIXELS_BGRA:
		bpp = 32;
		caps = gst_caps_new_simple("video/x-raw-rgb", "bpp", G_TYPE_INT, bpp, "depth", G_TYPE_INT, 32, "endianness",
		                           G_TYPE_INT, 4321, "red_mask", G_TYPE_INT, 0x0000ff00, "green_mask", G_TYPE_INT,
		                           0x00ff0000, "blue_mask", G_TYPE_INT, 0xff000000, "alpha_mask", G_TYPE_INT,
		                           0x000000ff, NULL);
		break;
	default:
		bpp = 32;
	...
	if(internalPixelFormat == OF_PIXELS_NATIVE) {
		caps =
		    gst_caps_from_string((mime + ",format={RGBA,BGRA,RGB,BGR,RGB16,GRAY8,YV12,I420,NV12,NV21,YUY2}").c_str());
	} else {
		string format = ofGstVideoUtils::getGstFormatName(internalPixelFormat);
		caps = gst_caps_new_simple(mime.c_str(), "format", G_TYPE_STRING, format.c_str(), NULL);
	}
#endif
...
#else
	/*auto gstPipeline = gst_parse_launch(("uridecodebin uri=" + name + " ! glcolorscale name=gl_filter ! appsink
	name=app_sink").c_str(),NULL); auto gstSink = gst_bin_get_by_name(GST_BIN(gstPipeline),"app_sink"); auto glfilter =
	gst_bin_get_by_name(GST_BIN(gstPipeline),"gl_filter"); gst_app_sink_set_caps(GST_APP_SINK(gstSink), caps);
	gst_caps_unref(caps);
...
bool ofGstVideoPlayer::load(string name) {
	...
	if(isInitialized()) {
		...
		return createPipeline(name) && videoUtils.startPipeline() && (bIsStream || allocate());
	}
}
...
bool ofGstVideoPlayer::allocate() {
	...
	if(GstPad *pad = gst_element_get_static_pad(videoUtils.getSink(), "sink")) {
		...
		if(gst_video_get_size(GST_PAD(pad), &width, &height)) {
			if(!videoUtils.allocate(width, height, internalPixelFormat))
				return false;
		} else {
		...
		if(GstCaps *caps = gst_pad_get_current_caps(GST_PAD(pad))) {
			...
			if(gst_video_info_from_caps(&info, caps)) {
				...
				if(format != internalPixelFormat) {
					ofLogVerbose("ofGstVideoPlayer") << "allocating as " << info.width << "x" << info.height << " "
					                                 << info.finfo->description << " " << info.finfo->name;
					internalPixelFormat = format;
				}
				if(!videoUtils.allocate(info.width, info.height, format))
					return false;
			} else {
				ofLogError("ofGstVideoPlayer") << "allocate(): couldn't query width and height";
				return false;
...
int ofGstVideoPlayer::getCurrentFrame() const {
	...
	if(floatRemainder > 0.5f)
		framePosInInt = framePosInInt + 1;
