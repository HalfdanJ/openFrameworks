...
public:
	...
	bool loadMovie(std::string path, ofPixelFormat format) {
		...
		// MEDIA CONVERSION
		// Get video properties from the stream's mediatype and apply to the grabber (otherwise we don't get an RGB
		// image)
		AM_MEDIA_TYPE mt;
		...
		case OF_PIXELS_RGBA:
			...
			break;
		default:
			ofLogError("DirectShowPlayer")
			    << "Trying to set unsupported format this is an internal bug, using default RGB";
			mt.subtype = MEDIASUBTYPE_RGB24;
		}
	...
	bool needsRBSwap(ofPixelFormat srcFormat, ofPixelFormat dstFormat) {
		return (srcFormat == OF_PIXELS_BGR || srcFormat == OF_PIXELS_BGRA) &&
		           (dstFormat == OF_PIXELS_RGB || dstFormat == OF_PIXELS_RGBA) ||
		       (srcFormat == OF_PIXELS_RGB || srcFormat == OF_PIXELS_RGBA) &&
		           (dstFormat == OF_PIXELS_BGR || dstFormat == OF_PIXELS_BGRA);
	}
...
protected:
	...
	long lvolume; // The volume level in 1/100ths dB Valid values range from -10,000 (silence) to 0 (full volume), 0 = 0
	              // dB -10000 = -100 dB
...
bool ofDirectShowPlayer::load(std::string path) {
	...
	if(!loadOk) {
		ofLogError("ofDirectShowPlayer") << " Cannot load video of this file type.  Make sure you have codecs "
		                                    "installed on your system.  OF recommends the free K-Lite Codec pack. ";
	}
	return loadOk;
}
