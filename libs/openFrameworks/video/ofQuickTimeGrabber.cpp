...
#include "ofUtils.h"
    ...
    (!defined(MAC_OS_X_VERSION_10_12) || MAC_OS_X_VERSION_MAX_ALLOWED < MAC_OS_X_VERSION_10_12)
...
bool ofQuickTimeGrabber::setPixelFormat(ofPixelFormat pixelFormat) {
	...
	ofLogWarning("ofQuickTimeGrabber") << "setPixelFormat(): requested pixel format " << pixelFormat
	                                   << " not supported";
	return false;
}
...
//---------------------------------
	...
	QTNewGWorldFromPtr(&(videogworld), k32ARGBPixelFormat, &(videoRect), nullptr, nullptr, 0, (offscreenGWorldPixels),
	                   4 * w);
#else
	QTNewGWorldFromPtr(&(videogworld), k24RGBPixelFormat, &(videoRect), nullptr, nullptr, 0, (pixels.getPixels()),
	                   3 * w);
#endif
	...
	if(didWeChooseADevice) {
		...
		if(!deviceIsSelected && bVerbose)
			ofLogError("ofQuickTimeGrabber")
			    << "initGrabber(): unable to open device[" << deviceID << "], will attempt other devices";
	}
	...
	if(attemptFramerate >= 0) {
		...
		if(err != noErr) {
			ofLogError("ofQuickTimeGrabber")
			    << "initGrabber: couldn't setting framerate to " << attemptFramerate << ": OSStatus " << err;
		}
	}
	...
	for(int i = 0; i < (*deviceList)->count; ++i) {
		...
		if(nameRec.flags != sgDeviceNameFlagDeviceUnavailable) {
			...
			for(int j = 0; j < numInputs; j++) {
				...
				ofLogNotice() << "device [" << deviceCount << "] " << p2cstr(pascalName) << " - "
				              << p2cstr(pascalNameInput);
...
bool ofQuickTimeGrabber::loadSettings() {
	...
	if(bGrabberInited != true || deviceName.length() == 0)
		return false;
	...
	if(mySGVideoSettings) {
		...
		if(err != noErr) {
			ofLogError("ofQuickTimeGrabber")
			    << "loadSettings(): couldn't set channel settings: ComponentResult " << err;
			return false;
		}
		DisposeUserData(mySGVideoSettings);
...
bool ofQuickTimeGrabber::qtInitSeqGrabber() {
	...
	if(bSgInited != true) {
		...
		if(sgCompID == nullptr) {
			ofLogError("ofQuickTimeGrabber")
			    << "qtInitSeqGrabber(): findNextComponent did not return a valid component";
			return false;
		}
		...
		if(gSeqGrabber == nullptr || err) {
			ofLogError("ofQuickTimeGrabber")
			    << "qtInitSeqGrabber(): couldn't get default sequence grabber component: OSErr " << err;
			return false;
		}
		...
		if(err != noErr) {
			ofLogError("ofQuickTimeGrabber")
			    << "qtInitSeqGrabber(): can't initialize sequence grabber component: OSErr " << err;
			return false;
		}
		...
		if(err != noErr) {
			ofLogError("ofQuickTimeGrabber")
			    << "qtInitSeqGrabber(): can't set the destination data reference: OSErr " << err;
			return false;
		}
...
bool ofQuickTimeGrabber::qtSelectDevice(int deviceNumber, bool didWeChooseADevice) {
	...
	for(int i = 0; i < numDevices; ++i) {
		...
		if(nameRec.flags != sgDeviceNameFlagDeviceUnavailable) {
				...
				if(deviceCount == deviceNumber || !didWeChooseADevice) {
					...
					if(successLevel > 0) {
						...
						if(successLevel == 2) {
							...
							ofLogWarning("ofQuickTimeGrabber")
							    << "device " << deviceName
							    << " opened with some paramater errors, should be fine though!";
						}
						...
						// if we selected a device in particular but failed we want to go through the whole list again -
						// starting from 0 and try any device. so we return false - and try one more time without a
						// preference
						if(didWeChooseADevice) {
							ofLogWarning("ofQuickTimeGrabber")
							    << "problems setting device [" << deviceNumber << "] " << p2cstr(pascalName) << " - "
							    << p2cstr(pascalNameInput) << " *****";
							return false;
						} else {
			...
			// ofLogError("ofQuickTimeGrabber") <<  "(unavailable) device [" << deviceCount << "] " <<
			// p2cstr(pascalName);
			deviceCount++;
		}
	}
