	...
	if(movieMedia && movieMediaHandler) {
		...
		if(err == noErr) {
			...
			} else /* working with non-MPEG-1/MPEG-2 media */
			{
				...
				if(err != noErr) {
					ofLogError("ofQtUtils")
					    << "MovieGetStaticFrameRate(): couldn't get static frame rate: OSErr " << err;
				}
				// assert(err == noErr);
			}
		}
...
OSErr IsMPEGMediaHandler(MediaHandler inMediaHandler, Boolean *outIsMPEG) {
	...
	/* is this the MPEG-1/MPEG-2 media handler? */
	return ((OSErr)MediaHasCharacteristic(inMediaHandler, kCharacteristicIsAnMpegTrack, outIsMPEG));
}
...
ComponentResult MPEGMediaGetStaticFrameRate(MediaHandler inMPEGMediaHandler, Fixed *outStaticFrameRate) {
	*outStaticFrameRate = 0;
	...
	ComponentResult err =
	    MediaGetPublicInfo(inMPEGMediaHandler, kMHInfoEncodedFrameRate, &encodedFrameRate, &encodedFrameRateSize);
	if(err == noErr) {
...
OSErr GetSettingsPreference(CFStringRef inKey, UserData *outUserData) {
	...
	theCFSettings = CFPreferencesCopyAppValue(inKey, kCFPreferencesCurrentApplication);
	if(theCFSettings) {
...
OSErr SaveSettingsPreference(CFStringRef inKey, UserData inUserData) {
	...
	if(noErr == err) {
		...
		if(noErr == err) {
			...
			if(theCFSettings) {
				CFPreferencesSetAppValue(inKey, theCFSettings, kCFPreferencesCurrentApplication);
