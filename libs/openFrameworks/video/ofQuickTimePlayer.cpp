...
bool createMovieFromPath(char *path, Movie &movie) {
	...
	if(result != noErr) {
		ofLogError("ofQuickTimePlayer")
		    << "createMovieFromPath(): couldn't load movie, NativePathNameToFSSpec failed: OSErr " << result;
		return false;
	}
	...
	if(result) {
		ofLogError("ofQuickTimePlayer") << "createMovieFromPath(): couldn't load movie, FSPathMakeRef failed: OSErr "
		                                << result;
		return false;
	}
	...
	if(result) {
		ofLogError("ofQuickTimePlayer")
		    << "createMovieFromPath(): couldn't load movie, FSGetCatalogInfo failed: OSErr ",
		    result;
		return false;
	}
#endif
	...
	if(result == noErr) {
		...
		if(result == noErr) {
			...
			ofLogError("ofQuickTimePlayer")
			    << "createMovieFromPath(): couldn't load movie, NewMovieFromFile failed: OSErr " << result;
			return false;
		}
	} else {
		ofLogError("ofQuickTimePlayer") << "createMovieFromPath(): couldn't load movie, OpenMovieFile failed: OSErr "
		                                << result;
		return false;
	}
...
bool createMovieFromURL(string urlIn, Movie &movie) {
	...
	if((err = MemError()) != noErr) {
		ofLogError("ofQuickTimePlayer") << "createMovieFromURL(): couldn't create url handle from \"" << urlIn
		                                << "\": OSErr " << err;
		return false;
	}
...
#if defined(TARGET_OSX) && defined(__BIG_ENDIAN__)
	QTNewGWorldFromPtr(&(offscreenGWorld), k32ARGBPixelFormat, &(movieRect), nullptr, nullptr, 0,
	                   (offscreenGWorldPixels), 4 * width);
#else
	QTNewGWorldFromPtr(&(offscreenGWorld), k24RGBPixelFormat, &(movieRect), nullptr, nullptr, 0, (pixels.getPixels()),
	                   3 * width);
#endif
...
//--------------------------------------
	...
	if(bDoWeAlreadyHaveAGworld) {
		...
		if(width == movieRect.right && height == movieRect.bottom) {
			SetMovieGWorld(moviePtr, offscreenGWorld, nil);
		} else {
...
//--------------------------------------
	...
	if(isLoaded()) {
		...
		default:
			myFlags &= ~loopTimeBase;
			myFlags &= ~palindromeLoopTimeBase;
			SetMoviePlayHints(moviePtr, 0L, hintsLoop | hintsPalindrome);
			break;
		}
...
bool ofQuickTimePlayer::setPixelFormat(ofPixelFormat pixelFormat) {
	...
	ofLogWarning("ofQuickTimePlayer") << "setPixelFormat(): requested pixel format " << pixelFormat
	                                  << " not supported, expecting OF_PIXELS_RGB";
	return false;
}
