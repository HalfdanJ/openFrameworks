...
ofGstUtils::ofGstUtils() {
	...
	if(!plugin_registered) {
		gst_plugin_register_static(GST_VERSION_MAJOR, GST_VERSION_MINOR, "appsink", (char *)"Element application sink",
...
GstFlowReturn ofGstUtils::preroll_cb(shared_ptr<GstSample> buffer) {
	...
	bIsMovieDone = false;
	if(appsink)
		return appsink->on_preroll(buffer);
	else
		return GST_FLOW_OK;
}
...
GstFlowReturn ofGstUtils::buffer_cb(shared_ptr<GstSample> buffer) {
	...
	bIsMovieDone = false;
	if(appsink)
		return appsink->on_buffer(buffer);
	else
		return GST_FLOW_OK;
}
...
bool ofGstUtils::setPipelineWithSink(GstElement *pipeline, GstElement *sink, bool isStream_) {
	...
	if(gstSink &&
	   string(gst_plugin_feature_get_name(GST_PLUGIN_FEATURE(gst_element_get_factory(gstSink)))) == "appsink") {
		isAppSink = true;
	} else {
		isAppSink = false;
	}
...
void ofGstUtils::setPosition(float pct) {
	...
	if(speed > 0) {
		if(!gst_element_seek(GST_ELEMENT(gstPipeline), speed, format, flags, GST_SEEK_TYPE_SET, pos, GST_SEEK_TYPE_SET,
		                     -1)) {
			ofLogWarning("ofGstUtils") << "setPosition(): unable to seek";
		}
	} else {
		if(!gst_element_seek(GST_ELEMENT(gstPipeline), speed, format, flags, GST_SEEK_TYPE_SET, 0, GST_SEEK_TYPE_SET,
		                     pos)) {
			ofLogWarning("ofGstUtils") << "setPosition(): unable to seek";
		}
	}
...
void ofGstUtils::setSpeed(float _speed) {
	...
	if(speed > 0) {
		if(!gst_element_seek(GST_ELEMENT(gstSink), speed, format, flags, GST_SEEK_TYPE_SET, pos, GST_SEEK_TYPE_SET,
		                     -1)) {
			ofLogWarning("ofGstUtils") << "setSpeed(): unable to change speed";
		}
	} else {
		if(!gst_element_seek(GST_ELEMENT(gstSink), speed, format, flags, GST_SEEK_TYPE_SET, 0, GST_SEEK_TYPE_SET,
		                     pos)) {
			ofLogWarning("ofGstUtils") << "setSpeed(): unable to change speed";
		}
	}
...
bool ofGstUtils::gstHandleMessage(GstBus *bus, GstMessage *msg) {
	if(appsink && appsink->on_message(msg))
		return true;
	...
	case GST_MESSAGE_ERROR: {
		...
		ofLogError("ofGstUtils") << "gstHandleMessage(): embedded video playback halted for plugin, module " << name
	...
	case GST_MESSAGE_EOS: {
		...
		case OF_LOOP_NORMAL: {
			...
			if(speed > 0) {
				if(!gst_element_seek(GST_ELEMENT(gstPipeline), speed, format, flags, GST_SEEK_TYPE_SET, 0,
				                     GST_SEEK_TYPE_SET, -1)) {
					ofLogWarning("ofGstUtils") << "gstHandleMessage(): unable to seek";
				}
			} else if(speed < 0) {
				if(!gst_element_seek(GST_ELEMENT(gstPipeline), speed, format, flags, GST_SEEK_TYPE_SET, 0,
				                     GST_SEEK_TYPE_SET, durationNanos - 1000000)) {
					ofLogWarning("ofGstUtils") << "gstHandleMessage(): unable to seek";
				}
			}
		} break;
		...
		case OF_LOOP_PALINDROME: {
			...
			if(pos > 0)
				...
				loopSpeed = speed;
			if(!gst_element_seek(GST_ELEMENT(gstPipeline), loopSpeed, GST_FORMAT_UNDEFINED, flags, GST_SEEK_TYPE_NONE,
		...
		if(!isFrameByFrame()) {
			...
			if(buffer) {
				if(pixels.isAllocated()) {
					...
					pixels.setFromExternalPixels(mapinfo.data, pixels.getWidth(), pixels.getHeight(),
...
bool ofGstVideoUtils::setPipeline(string pipeline, ofPixelFormat pixelFormat, bool isStream, int w, int h) {
	...
	case OF_PIXELS_MONO:
		...
		break;
	case OF_PIXELS_RGBA:
		caps = "video/x-raw-rgb, depth=24, bpp=32, endianness=4321, red_mask=0xff0000, green_mask=0x00ff00, "
		       "blue_mask=0x0000ff, alpha_mask=0x000000ff";
		break;
	case OF_PIXELS_BGRA:
		caps = "video/x-raw-rgb, depth=24, bpp=32, endianness=4321, red_mask=0x0000ff, green_mask=0x00ff00, "
		       "blue_mask=0xff0000, alpha_mask=0x000000ff";
		break;
	case OF_PIXELS_RGB:
	default:
		caps = "video/x-raw-rgb, depth=24, bpp=24, endianness=4321, red_mask=0xff0000, green_mask=0x00ff00, "
		       "blue_mask=0x0000ff, alpha_mask=0x000000ff";
		break;
	}
#else
	...
	string pipeline_string = pipeline + " ! appsink name=ofappsink enable-last-sample=0 caps=\"" + caps + "\"";
	...
	string pipeline_string =
	    pipeline +
	    " ! glcolorscale name=gl_filter ! appsink name=ofappsink enable-last-sample=0 caps=\"video/x-raw,format=RGBA\"";
	...
	glContext =
	    gst_gl_context_new_wrapped(glDisplay, (guintptr)ofGetGLXContext(), GST_GL_PLATFORM_GLX, GST_GL_API_OPENGL);
	...
	glContext =
	    gst_gl_context_new_wrapped(glDisplay, (guintptr)ofGetEGLContext(), GST_GL_PLATFORM_EGL, GST_GL_API_GLES2);
...
GstFlowReturn ofGstVideoUtils::process_buffer(shared_ptr<GstBuffer> _buffer) {
	...
	if(pixels.isAllocated() && pixels.getTotalBytes() != (int)size) {
		...
		if(stride == (pixels.getWidth() * pixels.getHeight() * pixels.getBytesPerPixel())) {
			ofLogError("ofGstVideoUtils") << "buffer_cb(): error on new buffer, buffer size: " << size
			                              << "!= init size: " << pixels.getTotalBytes();
	...
	if(pixels.isAllocated()) {
		backBuffer = _buffer;
		if(stride > 0) {
			backPixels.setFromAlignedPixels(GST_BUFFER_DATA(backBuffer.get()), pixels.getWidth(), pixels.getHeight(),
			                                pixels.getPixelFormat(), stride);
		} else {
			backPixels.setFromExternalPixels(GST_BUFFER_DATA(backBuffer.get()), pixels.getWidth(), pixels.getHeight(),
			                                 pixels.getPixelFormat());
			eventPixels.setFromExternalPixels(GST_BUFFER_DATA(backBuffer.get()), pixels.getWidth(), pixels.getHeight(),
			                                  pixels.getPixelFormat());
		}
		bBackPixelsChanged = true;
		mutex.unlock();
	...
	if(pixels.isAllocated()) {
		if(stride > 0) {
			if(pixels.getPixelFormat() == OF_PIXELS_I420) {
				GstVideoInfo v_info = getVideoInfo(sample.get());
				std::vector<size_t> strides{size_t(v_info.stride[0]), size_t(v_info.stride[1]),
				                            size_t(v_info.stride[2])};
				backPixels.setFromAlignedPixels(mapinfo.data, pixels.getWidth(), pixels.getHeight(),
				                                pixels.getPixelFormat(), strides);
			} else {
				backPixels.setFromAlignedPixels(mapinfo.data, pixels.getWidth(), pixels.getHeight(),
				                                pixels.getPixelFormat(), stride);
			}
		} else if(!copyPixels) {
			backPixels.setFromExternalPixels(mapinfo.data, pixels.getWidth(), pixels.getHeight(),
			                                 pixels.getPixelFormat());
			eventPixels.setFromExternalPixels(mapinfo.data, pixels.getWidth(), pixels.getHeight(),
			                                  pixels.getPixelFormat());
		} else {
