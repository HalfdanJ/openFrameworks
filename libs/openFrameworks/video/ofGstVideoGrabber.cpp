...
static void get_video_devices(ofGstCamData &cam_data) {
	...
	udev_list_entry_foreach(entry, list) {
		...
		ofLogNotice("ofGstVideoGrabber") << "Found device " << vendor_id << ":" << product_id
		...
		if(ok < 0) {
			...
			if(ok < 0) {
				ofLogWarning("ofGstVideoGrabber")
				    << "Error while probing v4l capabilities for " << dev_node << " " << strerror(errno);
				close(fd);
				continue;
			}
...
static void get_supported_framerates(ofGstVideoFormat &video_format, GstStructure &structure) {
	...
	if(GST_VALUE_HOLDS_FRACTION(framerates)) {
		...
		ofLog(OF_LOG_VERBOSE, "%d/%d ", framerate.numerator, framerate.denominator);
		...
		ofLogVerbose("ofGstVideoGrabber") << "get_supported_framerates(): from " << numerator_min << "/"
		                                  << denominator_max << " to " << numerator_max << "/" << denominator_min;
		...
		ofLogVerbose("ofGstVideoGrabber")
		    << "get_supported_framerates(): unknown GValue type " << G_VALUE_TYPE_NAME(framerates) << " for framerates";
	}
}
...
static void add_video_format(ofGstDevice &webcam_device, ofGstVideoFormat &video_format, GstStructure &format_structure,
                             int desired_framerate, ofPixelFormat desiredPixelFormat) {
	...
	if(i != -1) { // Resolution already added ?
		float new_framerate =
		    (float)video_format.choosen_framerate.numerator / video_format.choosen_framerate.denominator;
		...
		if(desired_framerate == -1) {
			if(new_framerate > curr_framerate) {
				...
				ofLogVerbose("ofGstVideoGrabber")
				    << "add_video_format(): non compressed format with same framerate, replacing existing format";
				webcam_device.video_formats[i] = video_format;
#else
			} else if((webcam_device.video_formats[i].mimetype == "video/x-raw-yuv" ||
			           webcam_device.video_formats[i].mimetype == "video/x-raw-rgb") &&
				...
				webcam_device.video_formats[i] = video_format;
			          ...
			          video_format.mimetype == "video/x-raw-rgb" && new_framerate == curr_framerate) {
				ofLogVerbose("ofGstVideoGrabber")
				    << "add_video_format(): rgb format with same framerate as yuv, replacing existing format";
				webcam_device.video_formats[i] = video_format;
#else
			} else if(webcam_device.video_formats[i].mimetype == "video/x-raw-rgb" &&
			          video_format.mimetype == "video/x-raw-yuv" && new_framerate == curr_framerate) {
				ofLogVerbose("ofGstVideoGrabber")
				    << "add_video_format(): rgb format with same framerate as yuv, replacing existing format";
				webcam_device.video_formats[i] = video_format;
#endif
...
static void add_video_format(ofGstDevice &webcam_device, ofGstVideoFormat &video_format, GstStructure &format_structure,
	...
	ofLogVerbose("ofGstVideoGrabber") << "add_video_format(): " << video_format.mimetype << " "
	                                  << video_format.format_name << video_format.width << "x" << video_format.height
	                                  << " "
	                                  << "videoformat: "
	...
	if(i != -1) { // Resolution already added ?
		...
		if(desired_framerate == -1) {
			...
			if(fabs(new_framerate - desired_framerate) < fabs(curr_framerate - desired_framerate)) {
				ofLogVerbose("ofGstVideoGrabber")
				    << "add_video_format(): more similar framerate replacing existing format";
				webcam_device.video_formats[i] = video_format;
			} else {
				ofLogVerbose("ofGstVideoGrabber") << "add_video_format(): already added, skipping";
			}
		...
		if((gst_video_format_from_string(webcam_device.video_formats[i].format_name.c_str()) ==
		        GST_VIDEO_FORMAT_ENCODED ||
		    gst_video_format_from_string(webcam_device.video_formats[i].format_name.c_str()) ==
		...
		else if(gst_video_format_from_string(webcam_device.video_formats[i].format_name.c_str()) !=
		            ofGstVideoUtils::getGstFormat(desiredPixelFormat) &&
		        gst_video_format_from_string(video_format.format_name.c_str()) ==
		            ofGstVideoUtils::getGstFormat(desiredPixelFormat) &&
		        new_framerate == curr_framerate) {
			ofLogVerbose("ofGstVideoGrabber")
			    << "add_video_format(): " << webcam_device.video_formats[i].format_name
			    << " format with same framerate as other format, replacing existing format";
			webcam_device.video_formats[i] = video_format;
...
static void get_supported_video_formats(ofGstDevice &webcam_device, GstCaps &caps, int desired_framerate,
                                        ofPixelFormat desiredPixelFormat) {
...
static void get_device_data(ofGstDevice &webcam_device, int desired_framerate, ofPixelFormat desiredPixelFormat) {
	string pipeline_desc =
	    webcam_device.gstreamer_src + " name=source device=" + webcam_device.video_device + " ! fakesink";
...
void ofGstVideoGrabber::setVerbose(bool bVerbose) {
	if(bVerbose)
		ofSetLogLevel("ofGstVideoGrabber", OF_LOG_VERBOSE);
	else
		ofSetLogLevel("ofGstVideoGrabber", OF_LOG_NOTICE);
}
...
vector<ofVideoDevice> ofGstVideoGrabber::listDevices() const {
	...
	for(unsigned i = 0; i < camData.webcam_devices.size(); i++) {
		...
		for(int j = 0; j < (int)camData.webcam_devices[i].video_formats.size(); j++) {
			devices[i].formats[j].pixelFormat =
			    ofPixelFormatFromGstFormat(camData.webcam_devices[i].video_formats[j].format_name);
			...
			for(int k = 0; k < (int)camData.webcam_devices[i].video_formats[j].framerates.size(); k++) {
				devices[i].formats[j].framerates[k] =
				    float(camData.webcam_devices[i].video_formats[j].framerates[k].numerator) /
				    float(camData.webcam_devices[i].video_formats[j].framerates[k].denominator);
			}
		}
		ofLogVerbose("ofGstVideoGrabber")
		    << "listDevices(): device " << i << ": " << camData.webcam_devices[i].video_device << ": "
		    << camData.webcam_devices[i].product_name;
	}
	return devices;
#else
...
ofGstVideoFormat &ofGstVideoGrabber::selectFormat(int w, int h, int desired_framerate,
	...
	for(unsigned i = 0; i < camData.webcam_devices[deviceID].video_formats.size(); i++) {
		if(camData.webcam_devices[deviceID].video_formats[i].width == w &&
		   camData.webcam_devices[deviceID].video_formats[i].height == h) {
			mostSimilarFormat = i;
			break;
		}
		int diff = abs(camData.webcam_devices[deviceID].video_formats[i].width +
		               camData.webcam_devices[deviceID].video_formats[i].height - w - h);
		if(diff < minDiff) {
			minDiff = diff;
			mostSimilarFormat = i;
		}
...
bool ofGstVideoGrabber::setup(int w, int h) {
	...
	if(internalPixelFormat != OF_PIXELS_NATIVE) {
		...
		ofLogNotice("ofGstVideoGrabber") << "initGrabber(): selected device: "
		                                 << camData.webcam_devices[deviceID].product_name;
		ofLogNotice("ofGstVideoGrabber") << "initGrabber(): selected format: " << format.width << "x" << format.height
		                                 << " " << format.mimetype << " " << format.format_name
		                                 << " framerate: " << format.choosen_framerate.numerator << "/"
	...
	gchar *pipeline_string = g_strdup_printf(
	    format_str_pipeline.c_str(), camData.webcam_devices[deviceID].gstreamer_src.c_str(),
	    camData.webcam_devices[deviceID].video_device.c_str(), format.mimetype.c_str(), format.width, format.height,
	    format.choosen_framerate.numerator, format.choosen_framerate.denominator, decodebin, scale);
#else
	...
	if(internalPixelFormat != OF_PIXELS_NATIVE) {
		...
		if(format.mimetype == "video/x-bayer") {
			decodebin = "! bayer2rgb ";
		          ...
		          gst_video_format_from_string(format.format_name.c_str()) == GST_VIDEO_FORMAT_UNKNOWN) {
			decodebin = "! decodebin ";
		}
		...
		if(format.format_name == "") {
			...
			pipeline_string = g_strdup_printf(
			    format_str_pipeline.c_str(), camData.webcam_devices[deviceID].gstreamer_src.c_str(),
			    camData.webcam_devices[deviceID].video_device.c_str(), format.mimetype.c_str(), format.width,
			    format.height, format.choosen_framerate.numerator, format.choosen_framerate.denominator,
			...
			pipeline_string = g_strdup_printf(
			    format_str_pipeline.c_str(), camData.webcam_devices[deviceID].gstreamer_src.c_str(),
			    camData.webcam_devices[deviceID].video_device.c_str(), format.mimetype.c_str(),
			    format.format_name.c_str(), format.width, format.height, format.choosen_framerate.numerator,
			    format.choosen_framerate.denominator, decodebin.c_str(), (scale + fix_v4l2_316).c_str());
		}
	} else {
		format_str_pipeline = "v4l2src name=video_source device=/dev/video%d ! video/x-raw,framerate=%d/1 %s";
		pipeline_string =
		    g_strdup_printf(format_str_pipeline.c_str(), deviceID, attemptFramerate, fix_v4l2_316.c_str());
	}
#endif
