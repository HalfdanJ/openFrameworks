...
void ofVideoPlayer::setPlayer(shared_ptr<ofBaseVideoPlayer> newPlayer) {
	player = newPlayer;
	setPixelFormat(internalPixelFormat); // this means that it will try to set the pixel format you have been using
	                                     // before. if the format is not supported ofVideoPlayer's internalPixelFormat
	                                     // will be updated to that of the player's
}
			...
			if(playerTex == nullptr) {
				...
				if(std::size_t(player->getWidth()) != 0 && std::size_t(player->getHeight()) != 0) {
					for(std::size_t i = 0; i < player->getPixels().getNumPlanes(); i++) {
						ofPixels plane = player->getPixels().getPlane(i);
						bool bDiffPixFormat =
						    (tex[i].isAllocated() && tex[i].texData.glInternalFormat !=
						                                 ofGetGLInternalFormatFromPixelFormat(plane.getPixelFormat()));
						if(bDiffPixFormat || !tex[i].isAllocated() ||
						   std::size_t(tex[i].getWidth()) != plane.getWidth() ||
						   std::size_t(tex[i].getHeight()) != plane.getHeight()) {
							tex[i].allocate(plane);
						}
...
void ofVideoPlayer::setVolume(float volume) {
	if(player) {
		if(volume > 1.0f) {
			ofLogWarning("ofVideoPlayer")
			    << "setVolume(): expected range is 0-1, limiting requested volume " << volume << " to 1.0";
			volume = 1.0f;
		}
...
void ofVideoPlayer::setUseTexture(bool bUse) {
	...
	if(bUse && player && !player->getTexturePtr() && std::size_t(getWidth()) != 0 && std::size_t(getHeight()) != 0) {
		for(std::size_t i = 0; i < player->getPixels().getNumPlanes(); i++) {
			...
			bool bDiffPixFormat =
			    (tex[i].isAllocated() &&
			     tex[i].texData.glInternalFormat != ofGetGLInternalFormatFromPixelFormat(plane.getPixelFormat()));
...
bool ofVideoPlayer::isInitialized() const {
	if(player) {
		return player->isInitialized() && (!bUseTexture || tex[0].isAllocated() ||
		                                   (player->getTexturePtr() && player->getTexturePtr()->isAllocated()));
	}
	return false;
}
