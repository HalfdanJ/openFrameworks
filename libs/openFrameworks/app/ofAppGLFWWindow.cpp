...
void ofAppGLFWWindow::setup(const ofGLFWWindowSettings &_settings) {
	...
	if(settings.windowMode == OF_GAME_MODE) {
		...
		if(count > settings.monitor) {
			windowP =
			    glfwCreateWindow(currentW, currentH, settings.title.c_str(), monitors[settings.monitor], sharedContext);
		} else {
		...
		windowP =
		    glfwCreateWindow(settings.getWidth(), settings.getHeight(), settings.title.c_str(), nullptr, sharedContext);
		if(!windowP) {
		...
		if(!iconSet) {
			...
			GIMP_IMAGE_RUN_LENGTH_DECODE(iconPixels.getData(), ofIconDebug.rle_pixel_data,
			                             iconPixels.getWidth() * iconPixels.getHeight(), ofIconDebug.bytes_per_pixel);
#else
			...
			GIMP_IMAGE_RUN_LENGTH_DECODE(iconPixels.getData(), ofIcon.rle_pixel_data,
			                             iconPixels.getWidth() * iconPixels.getHeight(), ofIcon.bytes_per_pixel);
#endif
...
#ifndef TARGET_OPENGLES
		static_cast<ofGLProgrammableRenderer *>(currentRenderer.get())
		    ->setup(settings.glVersionMajor, settings.glVersionMinor);
#else
	...
	xic = XCreateIC(xim, XNInputStyle, XIMPreeditNothing | XIMStatusNothing, XNClientWindow, getX11Window(),
	                XNFocusWindow, getX11Window(), NULL);
#endif
}
...
void ofAppGLFWWindow::setWindowIcon(const ofPixels &iconPixels) {
	...
	XChangeProperty(getX11Display(), getX11Window(), XInternAtom(getX11Display(), "_NET_WM_ICON", False), XA_CARDINAL,
	                32, PropModeReplace, (const unsigned char *)buffer.data(), length);
...
int ofAppGLFWWindow::getCurrentMonitor() {
	...
	for(int iC = 0; iC < numberOfMonitors; iC++) {
		...
		bool bPointMatch = xW >= monitorRect.getMinX() && yW >= monitorRect.getMinY() && xW < monitorRect.getMaxX() &&
		                   yW < monitorRect.getMaxY();
...
#include <X11/Xatom.h>
	...
	XChangeProperty(display, nativeWin, m_bypass_compositor, XA_CARDINAL, 32, PropModeReplace, (unsigned char *)&value,
	                1);
	...
	if(targetWindowMode == OF_FULLSCREEN) {
		...
		if(settings.multiMonitorFullScreen && monitorCount > 1) {
			...
			setWindowShape(allScreensSpace.width * pixelScreenCoordScale,
			               allScreensSpace.height * pixelScreenCoordScale);
			...
			// normally we would just call setWindowShape first, but on multi monitor you see the window bleed onto the
			// second monitor as it first changes shape and is then repositioned. this first moves it over in X, does
			// the screen resize and then by calling it again its set correctly in y.
		...
		NSWindow *cocoaWindow = glfwGetCocoaWindow(windowP);
		[cocoaWindow setStyleMask:NSTitledWindowMask | NSClosableWindowMask | NSMiniaturizableWindowMask |
	...
	if(targetWindowMode == OF_FULLSCREEN) {
		...
		DWORD STYLE = WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_MAXIMIZEBOX | WS_CLIPCHILDREN |
...
unsigned long keycodeToUnicode(ofAppGLFWWindow *window, int scancode, int modifier) {
	...
	typedef TISInputSourceRef (*pFnGetInputSource)(
	    void); // define function pointer that may return a input source ref, no arguments
	...
	//
	static pFnGetInputSource getInputSource = (pFnGetInputSource)CFBundleGetFunctionPointerForName(
	    tisBundle, CFSTR("TISCopyCurrentKeyboardLayoutInputSource"));
	static pFnGetKeyboardType getKeyboardType =
	    (pFnGetKeyboardType)CFBundleGetFunctionPointerForName(tisBundle, CFSTR("LMGetKbdType"));
	static pFnGetInputSourceProperty getInputSourceProperty =
	    (pFnGetInputSourceProperty)CFBundleGetFunctionPointerForName(tisBundle, CFSTR("TISGetInputSourceProperty"));

	static const TISInputSourceRef sourceRef =
	    getInputSource(); // note that for the first time, this creates a copy on the heap, then we're re-using it.

	static const CFStringRef *kPropertyUnicodeKeyLayoutData =
	    (CFStringRef *)CFBundleGetDataPointerForName(tisBundle, CFSTR("kTISPropertyUnicodeKeyLayoutData"));
	static const CFStringRef kTISPropertyUnicodeKeyLayoutData = *kPropertyUnicodeKeyLayoutData;
	static const CFDataRef UnicodeKeyLayoutData =
	    (CFDataRef)getInputSourceProperty(sourceRef, kTISPropertyUnicodeKeyLayoutData);
	...
	if(noErr == UCKeyTranslate(pKeyboardLayout, scancode, kUCKeyActionDisplay, mod_OSX, getKeyboardType(),
...
void ofAppGLFWWindow::motion_cb(GLFWwindow *windowP_, double x, double y) {
	...
	ofMouseEventArgs args(action, x * instance->pixelScreenCoordScale, y * instance->pixelScreenCoordScale,
	                      instance->buttonInUse, instance->events().getModifiers());
	instance->events().notifyMouseEvent(args);
}
...
void ofAppGLFWWindow::entry_cb(GLFWwindow *windowP_, int entered) {
	...
	ofMouseEventArgs args(action, instance->events().getMouseX(), instance->events().getMouseY(), instance->buttonInUse,
...
void ofAppGLFWWindow::scroll_cb(GLFWwindow *windowP_, double x, double y) {
	...
	ofMouseEventArgs args(ofMouseEventArgs::Scrolled, instance->events().getMouseX(), instance->events().getMouseY(),
	                      instance->buttonInUse, instance->events().getModifiers());
	args.scrollX = x;
	args.scrollY = y;
	instance->events().notifyMouseEvent(args);
...
void ofAppGLFWWindow::listMonitors() {
	...
	for(int i = 0; i < count; i++) {
		...
		ofLogNotice() << i << ": " << glfwGetMonitorName(monitor) << ", physical size: " << w << "x" << h << "mm at "
		              << x << ", " << y;
	}
}
