...
public:
	...
	OF_DEPRECATED_MSG("use ofGLFWWindowSettings to create the window instead",
	...
	OF_DEPRECATED_MSG(
	    ...
	    void setMultiDisplayFullscreen(
	        bool bMultiFullscreen)); // note this just enables the mode, you have to toggle fullscreen to activate it.
...
private:
	...
	int currentW, currentH; /// Extents of the window client area, which may be scaled by pixelsScreenCoordScale to map
	                        /// to physical framebuffer pixels.
	float pixelScreenCoordScale; /// Scale factor from virtual operating-system defined client area extents (as seen in
	                             /// currentW, currentH) to physical framebuffer pixel coordinates (as seen in windowW,
	                             /// windowH).
