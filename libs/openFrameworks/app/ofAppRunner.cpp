	...
	if(std::locale().name() == "C") {
		try {
			...
			if(ofToLower(std::locale("").name()).find("utf-8") == std::string::npos) {
				ofLogWarning("ofInit")
				    << "Couldn't set UTF-8 locale, string manipulation functions\n"
...
ofCoreEvents &ofEvents() {
	...
	if(window) {
		...
		if(!exiting()) {
			...
			ofLogError("ofEvents")
			    << "We'll return a void events instance to avoid crashes but somethings might not work";
			ofLogError("ofEvents") << "Set a breakpoint in " << __FILE__ << " line " << __LINE__
			                       << " to check where is the wrong call";
		}
		return noopEvents();
	}
}
