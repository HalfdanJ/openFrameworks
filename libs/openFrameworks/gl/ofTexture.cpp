...
void ofTexture::allocate(int w, int h, int glInternalFormat) {
	allocate(w, h, glInternalFormat, ofGetUsingArbTex(), ofGetGLFormatFromInternal(glInternalFormat),
	         ofGetGlTypeFromInternal(glInternalFormat));
}

//----------------------------------------------------------
void ofTexture::allocate(int w, int h, int glInternalFormat, bool bUseARBExtension) {
	allocate(w, h, glInternalFormat, bUseARBExtension, ofGetGLFormatFromInternal(glInternalFormat),
	         ofGetGlTypeFromInternal(glInternalFormat));
}
...
void ofTexture::allocate(const ofPixels &pix) {
	allocate(pix.getWidth(), pix.getHeight(), ofGetGlInternalFormat(pix), ofGetUsingArbTex(), ofGetGlFormat(pix),
	         ofGetGlType(pix));
	if((pix.getPixelFormat() == OF_PIXELS_GRAY || pix.getPixelFormat() == OF_PIXELS_GRAY_ALPHA) &&
...
void ofTexture::allocate(const ofPixels &pix, bool bUseARBExtention) {
	allocate(pix.getWidth(), pix.getHeight(), ofGetGlInternalFormat(pix), bUseARBExtention, ofGetGlFormat(pix),
	         ofGetGlType(pix));
	if((pix.getPixelFormat() == OF_PIXELS_GRAY || pix.getPixelFormat() == OF_PIXELS_GRAY_ALPHA) &&
	   ofIsGLProgrammableRenderer()) {
		setRGToRGBASwizzles(true);
...
void ofTexture::allocate(const ofTextureData &textureData) {
	allocate(textureData, ofGetGLFormatFromInternal(textureData.glInternalFormat),
	         ofGetGlTypeFromInternal(textureData.glInternalFormat));
}
...
void ofTexture::allocate(const ofTextureData &textureData, int glFormat, int pixelType) {
	...
	if(texData.textureTarget == GL_TEXTURE_2D) {
		...
		if(textureData.width <= 0.0 || textureData.height <= 0.0) {
			ofLogError("ofTexture") << "allocate(): ofTextureData has 0 width and/or height: " << textureData.width
	...
	if(texData.textureTarget == GL_TEXTURE_2D) {
		...
		glTexImage2D(texData.textureTarget, 0, texData.glInternalFormat, (GLint)texData.tex_w, (GLint)texData.tex_h, 0,
		             glFormat, pixelType, 0); // init to black...
...
void ofTexture::setRGToRGBASwizzles(bool rToRGBSwizzles) {
	...
	glBindTexture(texData.textureTarget, texData.textureID);
	if(rToRGBSwizzles) {
		if(texData.glInternalFormat == GL_R8 || texData.glInternalFormat == GL_R16 ||
		   texData.glInternalFormat == GL_R32F || texData.glInternalFormat == GL_DEPTH_COMPONENT

#ifndef TARGET_OPENGLES
		   || texData.glInternalFormat == GL_DEPTH_COMPONENT16 || texData.glInternalFormat == GL_DEPTH_COMPONENT24 ||
		...
		} else if(texData.glInternalFormat == GL_RG8 || texData.glInternalFormat == GL_RG16 ||
...
void ofTexture::loadData(const ofPixels &pix, int glFormat) {
	if(!isAllocated()) {
		allocate(pix.getWidth(), pix.getHeight(), ofGetGlInternalFormat(pix), ofGetUsingArbTex(), glFormat,
		         ofGetGlType(pix));
	}
	ofSetPixelStoreiAlignment(GL_UNPACK_ALIGNMENT, pix.getWidth(), pix.getBytesPerChannel(),
	                          ofGetNumChannelsFromGLFormat(glFormat));
...
void ofTexture::generateMipmap() {
	...
	if(!isGlGenerateMipmapAvailable && !ofGLCheckExtension("GL_EXT_framebuffer_object")) {
		...
		if(!versionWarningIssued)
			ofLogWarning() << "Your current OpenGL version does not support mipmap generation via glGenerateMipmap().";
...
#endif
	{
		...
		// glEnable(texData.textureTarget);	/// < uncomment this hack if you are unlucky enough to run an older ATI
		// card. See also: https://www.opengl.org/wiki/Common_Mistakes#Automatic_mipmap_generation
...
void ofTexture::loadScreenData(int x, int y, int w, int h) {
	...
	if(w > texData.tex_w || h > texData.tex_h) {
		ofLogError("ofTexture") << "loadScreenData(): " << w << "x" << h << " image data too big for " << texData.tex_w
		                        << "x " << texData.tex_h << " allocated texture, not uploading";
		return;
	}
...
void ofTexture::drawSubsection(const ofRectangle &drawBounds, const ofRectangle &subsectionBounds) const {
	drawSubsection(drawBounds.x, drawBounds.y, 0, drawBounds.width, drawBounds.height, subsectionBounds.x,
	               subsectionBounds.y, subsectionBounds.width, subsectionBounds.height);
}
...
//----------------------------------------------------------
void ofTexture::drawSubsection(float x, float y, float z, float w, float h, float sx, float sy, float sw,
                               float sh) const {
...
//------------------------------------
ofMesh ofTexture::getMeshForSubsection(float x, float y, float z, float w, float h, float sx, float sy, float sw,
                                       float sh, bool vflipped, ofRectMode rectMode) const {
	ofMesh quad;
	if(!texData.bAllocated) {
		return quad;
	}
...
void ofTexture::readToPixels(ofPixels &pixels) const {
	...
	ofSetPixelStoreiAlignment(GL_PACK_ALIGNMENT, pixels.getWidth(), pixels.getBytesPerChannel(),
...
void ofTexture::copyTo(ofBufferObject &buffer) const {
	ofSetPixelStoreiAlignment(GL_PACK_ALIGNMENT, getWidth(),
	                          ofGetBytesPerChannelFromGLType(ofGetGlTypeFromInternal(texData.glInternalFormat)),
	                          ofGetNumChannelsFromGLFormat(ofGetGLFormatFromInternal(texData.glInternalFormat)));
	buffer.bind(GL_PIXEL_PACK_BUFFER);
	glBindTexture(texData.textureTarget, texData.textureID);
	glGetTexImage(texData.textureTarget, 0, ofGetGLFormatFromInternal(texData.glInternalFormat),
	              ofGetGlTypeFromInternal(texData.glInternalFormat), 0);
	glBindTexture(texData.textureTarget, 0);
