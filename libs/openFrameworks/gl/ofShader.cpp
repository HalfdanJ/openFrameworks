...
static void releaseShader(GLuint program, GLuint id) {
	if(getShaderIds().find(id) != getShaderIds().end()) {
		...
		ofLogWarning("ofShader") << "releaseShader(): something's wrong here, releasing unknown shader id " << id
...
//--------------------------------------------------------------
ofShader::TransformFeedbackRangeBinding::TransformFeedbackRangeBinding(const ofBufferObject &buffer, GLuint offset,
                                                                       GLuint size)
    : offset(offset)
    , size(size)
    , buffer(buffer) {}
...
//--------------------------------------------------------------
bool ofShader::load(const std::filesystem::path &vertName, const std::filesystem::path &fragName,
                    const std::filesystem::path &geomName) {
	if(vertName.empty() == false)
		setupShaderFromFile(GL_VERTEX_SHADER, vertName);
	if(fragName.empty() == false)
		setupShaderFromFile(GL_FRAGMENT_SHADER, fragName);
#ifndef TARGET_OPENGLES
	if(geomName.empty() == false)
		setupShaderFromFile(GL_GEOMETRY_SHADER_EXT, geomName);
#endif
...
bool ofShader::setup(const TransformFeedbackSettings &settings) {
	...
	if(!settings.varyingsToCapture.empty()) {
		...
		std::transform(settings.varyingsToCapture.begin(), settings.varyingsToCapture.end(), varyings.begin(),
...
bool ofShader::setupShaderFromSource(ofShader::Source &&source) {
	...
	if(clearErrors != GL_NO_ERROR) {
		ofLogVerbose("ofShader")
		    << "setupShaderFromSource(): OpenGL error after checkAndCreateProgram() (probably harmless): error "
		    << clearErrors;
	}
	...
	for(auto &define : source.intDefines) {
		...
		source.expandedSource =
		    std::regex_replace(source.expandedSource, re_define, "#define " + name + " " + std::to_string(value));
	}
	...
	if(err != GL_NO_ERROR) {
		ofLogError("ofShader") << "setupShaderFromSource(): OpenGL generated error " << err
		                       << " trying to get the compile status for a " << nameForType(shader.source.type)
		                       << " shader, does your video card support this?";
		return false;
	}
	...
	if(status == GL_TRUE) {
		...
		ofLogError("ofShader") << "setupShaderFromSource(): "
		                       << nameForType(shader.source.type) + " shader failed to compile";
...
string ofShader::parseForIncludes(const string &source, vector<string> &included, int level,
	...
	auto match_pragma_include = [](const std::string &s_, std::string &filename_) -> bool {
		...
		if(f[0] == '<' && f[f.size() - 1] != '>')
			return false; //< mismatching brackets

		if((f[0] == '"' || f[0] == '\'') && (f[0] != f[f.size() - 1]))
			return false; // mismatching quotes
...
bool ofShader::checkProgramLinkStatus() {
	...
	if(err != GL_NO_ERROR) {
		ofLogError("ofShader")
		    << "checkProgramLinkStatus(): OpenGL generated error " << err
		    << " trying to get the program link status, does your video card support shader programs?";
		return false;
	}
	if(status == GL_TRUE)
...
bool ofShader::linkProgram() {
	if(shaders.empty()) {
		...
		for(auto it : shaders) {
			auto shader = it.second;
			if(shader.id > 0) {
				ofLogVerbose("ofShader") << "linkProgram(): attaching " << nameForType(it.first)
		...
		for(GLint i = 0; i < numUniforms; i++) {
			...
			if(location == -1)
				continue; // ignore uniform blocks
...
void ofShader::unload() {
	if(bLoaded) {
		for(auto it : shaders) {
			auto shader = it.second;
			if(shader.id) {
				ofLogVerbose("ofShader") << "unload(): detaching and deleting " << nameForType(shader.source.type)
...
void ofShader::setUniformMatrix3f(const string &name, const glm::mat3 &m, int count) const {
	if(bLoaded) {
		int loc = getUniformLocation(name);
		if(loc != -1)
			glUniformMatrix3fv(loc, count, GL_FALSE, glm::value_ptr(m));
	}
}
...
void ofShader::setUniformMatrix4f(const string &name, const glm::mat4 &m, int count) const {
	if(bLoaded) {
		int loc = getUniformLocation(name);
		if(loc != -1)
			glUniformMatrix4fv(loc, count, GL_FALSE, glm::value_ptr(m));
	}
}
...
GLint ofShader::getUniformBlockIndex(const string &name) const {
	...
	if(GLEW_ARB_uniform_buffer_object) {
		...
		ofLogError("ofShader::getUniformBlockIndex")
		    << "Sorry, it looks like you can't run 'ARB_uniform_buffer_object'";
		return -1;
	}
}
...
GLint ofShader::getUniformBlockBinding(const string &name) const {
	...
	if(GLEW_ARB_uniform_buffer_object) {
		...
		ofLogError("ofShader::getUniformBlockBinding")
		    << "Sorry, it looks like you can't run 'ARB_uniform_buffer_object'";
		return -1;
	}
}
...
void ofShader::printActiveUniformBlocks() const {
	if(GLEW_ARB_uniform_buffer_object) {
		...
		for(GLint i = 0; i < numUniformBlocks; i++) {
			...
			line << " @ index " << getUniformBlockIndex(uniformBlockName) << ", binding point " << blockBinding
			     << ", size " << blockDataSize << " bytes";
			ofLogNotice("ofShader") << line.str();
			line.str("");
		}
		delete[] uniformBlockName;
	} else {
		ofLogError("ofShader::printActiveUniformBlocks")
		    << "Sorry, it looks like you can't run 'ARB_uniform_buffer_object'";
	}
}
...
void ofShader::bindUniformBlock(GLuint binding, const string &name) const {
	if(bLoaded) {
		if(GLEW_ARB_uniform_buffer_object) {
			...
			ofLogError("ofShader::bindUniformBlock")
			    << "Sorry, it looks like you can't run 'ARB_uniform_buffer_object'";
		}
	}
}
