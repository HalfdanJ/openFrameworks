...
vector<weak_ptr<ofLight::Data>> &ofLightsData() {
	static vector<weak_ptr<ofLight::Data>> *lightsActive =
	    ofIsGLProgrammableRenderer() ? new vector<weak_ptr<ofLight::Data>> : new vector<weak_ptr<ofLight::Data>>(8);
	return *lightsActive;
}
...
void ofLight::setup() {
	if(data->glIndex == -1) {
		...
		if(bLightFound) {
			...
			ofLogError("ofLight") << "setup(): couldn't get active GL light, maximum number of "
			                      << ofLightsData().size() << " reached";
		}
	}
}
