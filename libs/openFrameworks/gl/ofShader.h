...
public:
	...
	void setGeometryInputType(
	    GLenum type); // type: GL_POINTS, GL_LINES, GL_LINES_ADJACENCY_EXT, GL_TRIANGLES, GL_TRIANGLES_ADJACENCY_EXT
	...
	enum defaultAttributes {
		POSITION_ATTRIBUTE = 0, // tig: was =1, and BOY, what a performance hog this was!!! see:
		                        // http://www.chromium.org/nativeclient/how-tos/3d-tips-and-best-practices
		COLOR_ATTRIBUTE,
		NORMAL_ATTRIBUTE,
		TEXCOORD_ATTRIBUTE,
	...
	/// @param type (GL_VERTEX_SHADER | GL_FRAGMENT_SHADER | GL_GEOMETRY_SHADER_EXT) the shader source you'd like to
	/// inspect.
...
private:
	...
	template<typename T> void setDefineConstantTemp(const std::string &name, T value);
	template<typename T> void setConstantTemp(const std::string &name, const std::string &type, T value);
	...
	static std::string parseForIncludes(const std::string &source,
	                                    const std::filesystem::path &sourceDirectoryPath = "");
	static std::string parseForIncludes(const std::string &source, std::vector<std::string> &included, int level = 0,
	                                    const std::filesystem::path &sourceDirectoryPath = "");
