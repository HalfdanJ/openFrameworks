...
public:
	...
	struct Settings {
		...
		bool depthStencilAsTexture; // use a texture instead of a renderbuffer for depth (useful to draw it or use it in
		                            // a shader later)
...
private:
	...
	GLuint fboTextures; // textures are attached to this (if MSAA is disabled, this is equal to fbo, otherwise it's a
	                    // new fbo)
	GLuint depthBuffer;
	...
	std::vector<GLenum>
	    activeDrawBuffers; ///< table of currently active color draw buffers, allocate() defaults it to size(textures),
	                       ///< with GL_COLOR_ATTACHMENT0..n as members, in order of allocation
