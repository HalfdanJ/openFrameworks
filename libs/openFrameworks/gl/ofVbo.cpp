...
static void releaseVAO(GLuint id) {
	if(getVAOIds().find(id) != getVAOIds().end()) {
		...
		ofLogWarning("ofVbo") << "releaseVAO(): something's wrong here, releasing unknown vertex array object id "
		                      << id;
...
//--------------------------------------------------------------
void ofVbo::VertexAttribute::setData(const float *attrib0x, int numCoords, int total, int usage, int stride,
                                     bool normalize) {
	if(!isAllocated()) {
		allocate();
	}
	...
	if(vaoChanged || !vaoSupported) {
		if(bUsingVerts) {
			if(!programmable) {
				...
				glVertexPointer(positionAttribute.numCoords, GL_FLOAT, positionAttribute.stride,
		...
		if(bUsingColors) {
			if(!programmable) {
				...
				glColorPointer(colorAttribute.numCoords, GL_FLOAT, colorAttribute.stride,
		...
		if(bUsingTexCoords) {
			if(!programmable) {
				...
				glTexCoordPointer(texCoordAttribute.numCoords, GL_FLOAT, texCoordAttribute.stride,
