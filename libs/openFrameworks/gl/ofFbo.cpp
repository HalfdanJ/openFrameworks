...
typedef void (*glFramebufferRenderbufferType)(GLenum target, GLenum attachment, GLenum renderbuffertarget,
                                              GLuint renderbuffer);
...
typedef void (*glRenderbufferStorageMultisampleType)(GLenum target, GLsizei samples, GLenum internalformat,
...
typedef void (*glFramebufferTexture2DType)(GLenum target, GLenum attachment, GLenum textarget, GLuint texture,
                                           GLint level);
	...
	if(!bglFunctionsInitialized) {
		if(ofIsGLProgrammableRenderer()) {
			...
			glRenderbufferStorageMultisample =
			    (glRenderbufferStorageMultisampleType)dlsym(RTLD_DEFAULT, "glRenderbufferStorageMultisample");
			...
			glRenderbufferStorageMultisample =
			    (glRenderbufferStorageMultisampleType)dlsym(RTLD_DEFAULT, "glRenderbufferStorageMultisampleOES");
...
void ofFbo::allocate(int width, int height, int internalformat, int numSamples) {
	...
	// we do this as the fbo and the settings object it contains could be created before the user had the chance to
	// disable or enable arb rect.
...
void ofFbo::allocate(Settings _settings) {
	...
	if(_settings.numSamples > maxSamples() && maxSamples() > -1) {
		ofLogWarning("ofFbo") << "allocate(): clamping numSamples " << _settings.numSamples << " to maxSamples "
		                      << maxSamples() << " for frame buffer object" << fbo;
		_settings.numSamples = maxSamples();
	}

	if(_settings.depthStencilAsTexture && _settings.numSamples) {
		ofLogWarning("ofFbo") << "allocate(): multisampling not supported with depthStencilAsTexture, setting 0 "
		                         "samples for frame buffer object "
		                      << fbo;
	...
	if(!_settings.depthStencilAsTexture) {
		if(_settings.useDepth && _settings.useStencil) {
			stencilBuffer = depthBuffer =
			    createAndAttachRenderbuffer(_settings.depthStencilInternalFormat, depthAttachment);
			retainRB(stencilBuffer);
			retainRB(depthBuffer);
		} else if(_settings.useDepth) {
		...
		if(_settings.useDepth || _settings.useStencil) {
			createAndAttachDepthStencilTexture(_settings.textureTarget, _settings.depthStencilInternalFormat,
			                                   depthAttachment);
#ifdef TARGET_OPENGLES
			...
			if(_settings.useDepth && _settings.useStencil) {
				glFramebufferTexture2D(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_TEXTURE_2D,
				                       depthBufferTex.texData.textureID, 0);
			}
#endif
		}
	}
...
// if we want MSAA, create a new fbo for textures
	...
	if(_settings.colorFormats.size() > 0) {
		...
		for(int i = 0; i < _settings.numColorbuffers; i++)
			createAndAttachTexture(_settings.internalformat, i);
	...
	if(settings != _settings) ofLogWarning("ofFbo") << "allocation not complete, passed settings not equal to created
	ones, this is an internal OF bug";
...
GLuint ofFbo::createAndAttachRenderbuffer(GLenum internalFormat, GLenum attachmentPoint) {
	...
	if(settings.numSamples == 0) {
		...
		glRenderbufferStorageMultisample(GL_RENDERBUFFER, settings.numSamples, internalFormat, settings.width,
		                                 settings.height);
	}
#else
...
void ofFbo::attachTexture(ofTexture &tex, GLenum internalFormat, GLenum attachmentPoint) {
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + attachmentPoint, tex.texData.textureTarget,
	                       tex.texData.textureID, 0);
...
void ofFbo::createAndAttachDepthStencilTexture(GLenum target, GLint internalformat, GLenum attachment,
                                               GLenum transferFormat, GLenum transferType) {
...
void ofFbo::setActiveDrawBuffers(const vector<int> &ids) {
	...
	activeDrawBuffers.clear();
	activeDrawBuffers.resize(
	    numBuffers,
	    GL_NONE); // we initialise the vector with GL_NONE, so a buffer will not be written to unless activated.
	for(int i = 0; i < (int)ids.size(); i++) {
		...
		if(id < getNumTextures()) {
			...
			ofLogWarning("ofFbo") << "setActiveDrawBuffers(): fbo " << fbo << " couldn't set texture " << i << ", only "
			                      << getNumTextures() << "allocated";
		}
	}
...
void ofFbo::copyTo(ofBufferObject &buffer) const {
	...
	glReadPixels(0, 0, settings.width, settings.height, ofGetGLFormatFromInternal(settings.internalformat),
	             ofGetGlTypeFromInternal(settings.internalformat), NULL);
...
void ofFbo::draw(float x, float y, float width, float height) const {
	if(!bIsAllocated || settings.numColorbuffers == 0)
		return;
...
ofTexture &ofFbo::getDepthTexture() {
	if(!settings.depthStencilAsTexture) {
		ofLogError("ofFbo") << "getDepthTexture(): frame buffer object " << fbo
		                    << " not allocated with depthStencilAsTexture";
	}
