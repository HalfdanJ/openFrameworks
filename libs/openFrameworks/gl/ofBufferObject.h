...
public:
	...
	template<typename T> void allocate(const std::vector<T> &data, GLenum usage) {
		allocate(data.size() * sizeof(T), &data[0], usage);
	}

	template<typename T> void allocate(const ofPixels_<T> &data, GLenum usage) {
	...
	template<typename T> void setData(const ofPixels_<T> &data, GLenum usage) {
	...
	/// of the passed vector and size of the type
	template<typename T> void updateData(GLintptr offset, const std::vector<T> &data) {
	...
	template<typename T> T *map(GLenum access) {
	...
	template<typename T> T *mapRange(GLintptr offset, GLsizeiptr length, GLenum access) {
