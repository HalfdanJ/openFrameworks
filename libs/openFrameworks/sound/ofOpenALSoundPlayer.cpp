...
// ----------------------------------------------------------------------------
bool ofOpenALSoundPlayer::sfReadFile(const std::filesystem::path &path, vector<short> &buffer,
	...
	if(subformat == SF_FORMAT_FLOAT || subformat == SF_FORMAT_DOUBLE) {
		...
		if(frames_read < sfInfo.frames) {
			ofLogError("ofOpenALSoundPlayer")
			    << "sfReadFile(): read " << frames_read << " frames from buffer, expected " << sfInfo.frames
			    << " for \"" << path << "\"";
			return false;
		}
		...
		if(frames_read < sfInfo.frames) {
			ofLogError("ofOpenALSoundPlayer")
			    << "sfReadFile(): read " << frames_read << " frames from fft buffer, expected " << sfInfo.frames
			    << " for \"" << path << "\"";
			return false;
		}
	}
...
#ifdef OF_USING_MPG123
//------------------------------------------------------------
bool ofOpenALSoundPlayer::mpg123ReadFile(const std::filesystem::path &path, vector<short> &buffer,
                                         vector<float> &fftAuxBuffer) {
	int err = MPG123_OK;
	...
	if(encoding != MPG123_ENC_SIGNED_16) {
		ofLogError("ofOpenALSoundPlayer")
		    << "mpg123ReadFile(): " << getMpg123EncodingString(encoding) << " encoding for \"" << path << "\""
		    << " unsupported, expecting MPG123_ENC_SIGNED_16";
		return false;
	}
	samplerate = rate;
	...
	while(mpg123_read(f, (unsigned char *)&buffer[buffer.size() - buffer_size / 2], buffer_size, &done) !=
	      MPG123_DONE) {
...
//------------------------------------------------------------
bool ofOpenALSoundPlayer::sfStream(const std::filesystem::path &path, vector<short> &buffer,
                                   vector<float> &fftAuxBuffer) {
	if(!streamf) {
		SF_INFO sfInfo;
	...
	if(speed > 1)
		curr_buffer_size *= (int)round(speed);
...
#ifdef OF_USING_MPG123
//------------------------------------------------------------
bool ofOpenALSoundPlayer::mpg123Stream(const std::filesystem::path &path, vector<short> &buffer,
                                       vector<float> &fftAuxBuffer) {
	if(!mp3streamf) {
		int err = MPG123_OK;
		...
		if(stream_encoding != MPG123_ENC_SIGNED_16) {
			ofLogError("ofOpenALSoundPlayer")
			    << "mpg123Stream(): " << getMpg123EncodingString(stream_encoding) << " encoding for \"" << path << "\""
			    << " unsupported, expecting MPG123_ENC_SIGNED_16";
			return false;
		}
		samplerate = rate;
...
bool ofOpenALSoundPlayer::stream(const std::filesystem::path &fileName, vector<short> &buffer) {
	...
	if(ofFilePath::getFileExt(fileName) == "mp3" || ofFilePath::getFileExt(fileName) == "MP3" || mp3streamf) {
		if(!mpg123Stream(fileName, buffer, fftAuxBuffer))
			return false;
	} else
#endif
	    if(!sfStream(fileName, buffer, fftAuxBuffer))
		return false;
...
bool ofOpenALSoundPlayer::readFile(const std::filesystem::path &fileName, vector<short> &buffer) {
	...
	if(ofFilePath::getFileExt(fileName) != "mp3" && ofFilePath::getFileExt(fileName) != "MP3") {
		if(!sfReadFile(fileName, buffer, fftAuxBuffer))
			return false;
	} else {
		if(!mpg123ReadFile(fileName, buffer, fftAuxBuffer))
			return false;
	}
#else
	if(!sfReadFile(fileName, buffer, fftAuxBuffer))
		...
		if(isStreaming) {
			for(int s = 0; s < 2; s++) {
				for(int i = 0; i < channels; i++) {
					...
					alGetError(); // Clear error.
					alBufferData(buffers[s * 2 + i], format, &multibuffer[i][0], buffer.size() / channels * 2,
					             samplerate);
					err = alGetError();
					if(err != AL_NO_ERROR) {
						ofLogError("ofOpenALSoundPlayer")
						    << "loadSound(): couldn't create stereo buffers for \"" << fileName << "\": " << (int)err
			...
			for(int i = 0; i < channels; i++) {
				...
				err = alGetError();
				if(err != AL_NO_ERROR) {
					ofLogError("ofOpenALSoundPlayer") << "loadSound(): couldn't create stereo buffers for \""
					                                  << fileName << "\": " << (int)err << " " << getALErrorString(err);
					return false;
				}
			...
			while(processed--) {
				...
				if(channels > 1) {
					for(int j = 0; j < channels; j++) {
						...
						alBufferData(albuffer, AL_FORMAT_MONO16, &multibuffer[j][0], buffer.size() * 2 / channels,
						             samplerate);
...
void ofOpenALSoundPlayer::setLoop(bool bLp) {
	if(bMultiPlay)
		return; // no looping on multiplay
	bLoop = bLp;
	if(isStreaming)
		return;
	for(int i = 0; i < (int)sources.size(); i++) {
...
void ofOpenALSoundPlayer::play() {
	...
	if(bMultiPlay && !bLoop) {
		...
		if(err != AL_NO_ERROR) {
			ofLogError("ofOpenALSoundPlayer")
			    << "play(): couldn't create multiplay stereo sources: " << (int)err << " " << getALErrorString(err);
			return;
		}
		for(int i = 0; i < channels; i++) {
		...
		if(err != AL_NO_ERROR) {
			ofLogError("ofOpenALSoundPlayer")
			    << "play(): couldn't assign multiplay buffers: " << (int)err << " " << getALErrorString(err);
			return;
		}
	}
...
void ofOpenALSoundPlayer::initSystemFFT(int bands) {
	if(int(systemBins.size()) == bands)
		return;
	int signalSize = (bands - 1) * 2;
	if(systemFftCfg != 0)
		kiss_fftr_free(systemFftCfg);
...
float *ofOpenALSoundPlayer::getSystemSpectrum(int bands) {
	...
	for(int i = 0; i < bands; i++) {
		systemBins[i] +=
		    sqrtf(systemCx_out[i].r * systemCx_out[i].r + systemCx_out[i].i * systemCx_out[i].i) * normalizer;
	}
	return &systemBins[0];
}
