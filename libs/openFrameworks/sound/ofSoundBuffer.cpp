...
ofSoundBuffer::ofSoundBuffer(short *shortBuffer, std::size_t numFrames, std::size_t numChannels,
...
void ofSoundBuffer::copyFrom(const short *shortBuffer, std::size_t numFrames, std::size_t numChannels,
                             unsigned int sampleRate) {
...
void ofSoundBuffer::copyFrom(const float *floatBuffer, std::size_t numFrames, std::size_t numChannels,
                             unsigned int sampleRate) {
	this->channels = numChannels;
	setSampleRate(samplerate);
...
bool ofSoundBuffer::checkSizeAndChannelsConsistency(const std::string &_function) {
	...
	if((size() % channels) != 0) {
		ofLogWarning("ofSoundBuffer") << function << "channel count " << channels
		                              << " is not consistent with sample count " << size() << " (non-zero remainder)";
		return false;
	}
	return true;
}
...
void ofSoundBuffer::stereoPan(float left, float right) {
	if(channels != 2) {
		ofLogWarning("ofSoundBuffer") << "stereoPan called on a buffer with " << channels
		                              << " channels, only works with 2 channels";
		return;
	}
...
void ofSoundBuffer::copyTo(ofSoundBuffer &soundBuffer, std::size_t nFrames, std::size_t outChannels,
                           std::size_t fromFrame, bool loop) const {
...
void ofSoundBuffer::addTo(ofSoundBuffer &soundBuffer, std::size_t nFrames, std::size_t outChannels,
                          std::size_t fromFrame, bool loop) const {
	soundBuffer.resize(nFrames * outChannels);
	soundBuffer.setNumChannels(outChannels);
	soundBuffer.setSampleRate(samplerate);
...
void ofSoundBuffer::copyTo(float *outBuffer, std::size_t nFrames, std::size_t outChannels, std::size_t fromFrame,
                           bool loop) const {
...
void ofSoundBuffer::addTo(float *outBuffer, std::size_t nFrames, std::size_t outChannels, std::size_t fromFrame,
                          bool loop) const {
	// figure out how many frames we can copy before we need to stop or loop
	std::size_t nFramesToCopy = nFrames;
	if((fromFrame + nFrames) >= this->getNumFrames()) {
...
static bool prepareBufferForResampling(const ofSoundBuffer &in, ofSoundBuffer &out, std::size_t numFrames) {
	...
	if(totalOutBufferSize < out.getBuffer().max_size()) {
		...
		ofLogError("ofSoundUtils") << "resampling would create a buffer size of " << totalOutBufferSize
		                           << " (too large for std::vector)";
		return false;
	}
...
void ofSoundBuffer::linearResampleTo(ofSoundBuffer &outBuffer, std::size_t fromFrame, std::size_t numFrames,
...
void ofSoundBuffer::hermiteResampleTo(ofSoundBuffer &outBuffer, std::size_t fromFrame, std::size_t numFrames,
                                      float speed, bool loop) const {
...
void ofSoundBuffer::resampleTo(ofSoundBuffer &buffer, std::size_t fromFrame, std::size_t numFrames, float speed,
                               bool loop, InterpolationAlgorithm algorithm) const {
	switch(algorithm) {
	case Linear:
...
void ofSoundBuffer::getChannel(ofSoundBuffer &targetBuffer, std::size_t sourceChannel) const {
	...
	if(sourceChannel >= channels) {
		ofLogWarning("ofSoundBuffer") << "getChannel requested channel " << sourceChannel << " but we only have "
		                              << channels << " channels. clamping channel to " << channels - 1;
