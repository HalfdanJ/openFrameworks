...
float *ofFmodSoundGetSpectrum(int nBands) {
	...
	if(nBands > 8192) {
		ofLogWarning("ofFmodSoundPlayer")
		    << "ofFmodGetSpectrum(): requested number of bands " << nBands << ", using maximum of 8192";
		nBands = 8192;
	} else if(nBands <= 0) {
		ofLogWarning("ofFmodSoundPlayer")
		    << "ofFmodGetSpectrum(): requested number of bands " << nBands << ", using minimum of 1";
		nBands = 1;
	...
	if(nBandsToGet < 64)
		nBandsToGet = 64; // can't seem to get fft of 32, etc from fmodex
	...
	if(nBandsToGet == nBands) {
		...
		for(int i = 0; i < nBands; i++) {
			...
			if(fftInterpValues_[i] > 1)
				fftInterpValues_[i] = 1; // this seems "wrong"
		}
	}
...
bool ofFmodSoundPlayer::load(const std::filesystem::path &_fileName, bool stream) {
	...
	if(stream)
		fmodFlags = FMOD_SOFTWARE | FMOD_CREATESTREAM;
