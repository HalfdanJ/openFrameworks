...
void ofSerial::enumerateWin32Ports() {
	...
	if(hDevInfo) {
		...
		while(TRUE) {
			...
			if(SetupDiGetDeviceRegistryPropertyA(hDevInfo, &DeviceInterfaceData, SPDRP_FRIENDLYNAME, &dataType, dataBuf,
			                                     sizeof(dataBuf), &actualSize)) {
...
void ofSerial::buildDeviceList() {
	...
	for(int i = 0; i < nPorts; i++) {
		// NOTE: we give the short port name for both as that is what the user should pass and the short name is more
		// friendly
...
bool ofSerial::setup(int deviceNumber, int baud) {
	...
	if(deviceNumber < (int)devices.size()) {
		...
		ofLogError("ofSerial") << "couldn't find device " << deviceNumber << ", only " << devices.size()
		                       << " devices found";
		return false;
	}
}
...
bool ofSerial::setup(string portName, int baud) {
	...
	if(!SetCommState(hComm, &cfg.dcb)) {
		ofLogError("ofSerial") << "setup(): couldn't set comm state: " << cfg.dcb.BaudRate << " bps, xio "
		                       << cfg.dcb.fInX << "/" << cfg.dcb.fOutX;
	}
