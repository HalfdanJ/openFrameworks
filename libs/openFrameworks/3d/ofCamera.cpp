...
//----------------------------------------
void ofCamera::setupOffAxisViewPortal(const glm::vec3 &topLeft, const glm::vec3 &bottomLeft,
...
glm::mat4 ofCamera::getProjectionMatrix(const ofRectangle &viewport) const {
	...
	if(isOrtho) {
		return glm::ortho(-viewport.width / 2, +viewport.width / 2, -viewport.height / 2, +viewport.height / 2,
		                  nearClip, farClip);
	} else {
